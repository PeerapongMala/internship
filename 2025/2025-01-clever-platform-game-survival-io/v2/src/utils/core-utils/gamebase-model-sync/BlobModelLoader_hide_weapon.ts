...
import { StoreModelFileMethods } from './StoreModelFileMethods';
...
export function LoadFBXCharacter(
  ...
  const fallbackModelURL = 'set1_character1_level1'; // Define fallback file path
...
export async function LoadCharacter(
  ...
  const fallbackModelURL = 'set1_character1_level1'; // Direct file path for fallback
  ...
  try {
    ...
    if (modelBlob) {
      ...
      if (!isRetry) {
        ...
        actualModelUsed = 'set1_character1_level1';
    ...
    if (!isRetry) {
      ...
      sourceURL = fallbackModelURL;
      actualModelUsed = 'FallbackModelFile';
      objectUrlToRevoke = null;
    }
  }
  ...
  const handleLoadSuccess = (character: THREE.Group) => {
    ...
    // Debug: Log skeleton bones of loaded character
    console.log(`🎭 Loaded character model: ${actualModelUsed}`);
