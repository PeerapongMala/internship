...
import { isAnimationCompatible } from './animation-utils';
          ...
          (character: THREE.Group, actualModelUsed: string) => {
            ...
            loadFBXAnimation(
              ...
              (animationClip: THREE.AnimationClip) => {
                ...
                if (resultClip && mixer) {
                  // Validate animation compatibility before applying
                  if (!isAnimationCompatible(character, resultClip)) {
                    console.error('⚠️ Animation skeleton mismatch detected!');
                    console.error(`Character model: ${actualModelUsed}`);
                    console.error(`Animation file: ${animationBasedOnModelSrc}`);
                    // Still try to play but with warning
                  }
