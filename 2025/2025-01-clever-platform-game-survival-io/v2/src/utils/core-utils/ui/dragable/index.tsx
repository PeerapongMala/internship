...
const FloatingSnapButton = ({ children }: { children?: React.ReactNode }) => {
  // const { t, i18n } = useTranslation(['global']);
  const [currentScale] = useState<number>(1);
  ...
  const [boundingRect] = useState<{ left: number; top: number }>({
    left: 0,
    top: 0,
  });
  ...
  const [scenarioSize, setScenarioSize] = useState<{ width: number; height: number }>({
    width: 1280,
    height: 720,
  });
      ...
      <div className="absolute top-0 left-0 h-0 w-0">
        ...
        >
          <div
            ...
            // className={cn(
            className={
              'border-box bg-ghost/80 border-ghost-border relative cursor-move rounded-full border-b-4 select-none'
            }
            style={{
              boxShadow:
...
export default FloatingSnapButton;
