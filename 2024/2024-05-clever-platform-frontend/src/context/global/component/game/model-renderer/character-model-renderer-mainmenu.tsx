...
import { GCAThreeModel } from '../three-d-model/character-animation-controller-mainmenu';
...
export function ThreeModelRenderer({
  ...
  useEffect(() => {
    ...
    // Clean up on unmount
    return () => {
      console.log('Cleanup: removing model from scene (Main Menu Renderer)');
  ...
  return (
    <canvas
      ...
      className={
        className ||
        `w-full h-full absolute -bottom-36 -left-[45%] z-[-10] ${modelLoaded ? 'opacity-100' : 'opacity-0'}`
      }
      style={style}
    />
  );
