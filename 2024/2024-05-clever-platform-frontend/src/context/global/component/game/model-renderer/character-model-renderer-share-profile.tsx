...
import { GCAThreeModel } from '../three-d-model/character-animation-controller-share-profile';
  ...
  useEffect(() => {
    ...
    if (canvasElement) {
      ...
      if (modelSrc) {
        ...
        model.Start({
          modelSrc,
          scene: sceneRef.current, // Pass the scene
          ...
          onError: (error: string) => {
            // Only update state if this is still the current model
            if (currentModelSrcRef.current === modelSrc) {
    ...
    return () => {
      console.log('Cleanup: removing model from scene (Share Profile Renderer)');
      currentModelSrcRef.current = undefined; // Invalidate current model ref

      // Use cancelAndClear to properly clean up and reject pending loads
      model?.cancelAndClear(sceneRef.current);
