...
} from '@global/helper/device-detector';
import { MemoryMonitor } from '@global/helper/memory-monitor';
...
const WCARenderMemoryInfo = () => {
  ...
  const checkDebugMode = (): void => {
    ...
    if (enableDebugModeParam !== null) {
      const enableDebugMode = enableDebugModeParam === 'true';
  ...
  const getPerformanceInfo = (): Omit<PerformanceInfo, 'fps' | 'frameTime'> => {
    ...
    try {
      ...
      activeListeners += document.querySelectorAll('button, a, input, select, textarea').length;
    } catch (e) {
      activeListeners = 0;
    }
    ...
    <div className="space-y-2">
      ...
      <div className="border-b border-gray-600 pb-2">
        ...
        {performanceInfo.fps < 30 && performanceInfo.fps > 0 && (
          <div className="text-red-300">⚠️ Low FPS detected ({performanceInfo.fps})</div>
        )}
        {performanceInfo.frameTime > 33 && (
          <div className="text-yellow-300">⚠️ High frame time ({performanceInfo.frameTime.toFixed(1)}ms)</div>
        )}
        {performanceInfo.domNodes > 10000 && (
          <div className="text-orange-300">⚠️ High DOM complexity ({performanceInfo.domNodes.toLocaleString()} nodes)</div>
        )}
        {memoryInfo.percentage > 70 && (
          <div className="text-red-300">⚠️ High memory usage ({memoryInfo.percentage.toFixed(1)}%)</div>
        )}
        {performanceInfo.devicePixelRatio > 2 && (
          <div className="text-yellow-300">⚠️ High pixel density may impact performance (@{performanceInfo.devicePixelRatio}x)</div>
        )}
        {performanceInfo.webglVersion === 'None' && (
          <div className="text-red-300">❌ WebGL not available</div>
        )}
        {(navigator.hardwareConcurrency || 4) <= 2 && (
          <div className="text-orange-300">⚠️ Limited CPU cores ({navigator.hardwareConcurrency || 'Unknown'})</div>
        )}
        {performanceInfo.batteryLevel !== undefined && performanceInfo.batteryLevel < 20 && (
          <div className="text-red-300">🔋 Low battery may affect performance ({performanceInfo.batteryLevel.toFixed(0)}%)</div>
        )}
        {!navigator.onLine && (
          <div className="text-red-300">📶 Offline mode</div>
        )}
        ...
        {performanceInfo.imageCount > 100 && (
          <div className="text-yellow-300">🖼️ High image count may impact memory ({performanceInfo.imageCount})</div>
        )}
        {performanceInfo.canvasCount > 5 && (
          <div className="text-yellow-300">🎨 Multiple canvas elements detected ({performanceInfo.canvasCount})</div>
        )}
      ...
      <div>
        <div className="font-bold text-orange-400">Debug Controls</div>
        <div className="flex items-center justify-between mt-1">
          <label className="text-xs">Active Particle:</label>
          <input
            type="checkbox"
            checked={settings?.enableParticle || false}
            onChange={(e) =>
              StoreGlobalPersist.MethodGet().updateSettings({
                enableParticle: e.target.checked,
              })
            }
            className="ml-2"
          />
        ...
        <div className="flex items-center justify-between mt-1">
          <label className="text-xs">Gameplay Model Renderer:</label>
          <input
            type="checkbox"
            checked={settings?.enableGameplayModelRenderer || false}
            onChange={(e) =>
              StoreGlobalPersist.MethodGet().updateSettings({
                enableGameplayModelRenderer: e.target.checked,
              })
            }
            className="ml-2"
          />
        ...
        <div className="flex items-center justify-between mt-1">
          <label className="text-xs">Debug FPS Tracking:</label>
          <input
            type="checkbox"
            checked={showMemoryInfo}
            onChange={(e) =>
              StoreGlobalPersist.MethodGet().setShowMemoryInfo(e.target.checked)
            }
            className="ml-2"
          />
        </div>
  ...
  return (
    <>
      ...
      {showMemoryInfo && (
        <div
          className={`absolute bg-black bg-opacity-90 text-white p-3 rounded text-xs z-50 font-mono max-w-sm select-none ${
...
export default WCARenderMemoryInfo;
