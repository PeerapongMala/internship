...
const DomainJSX = () => {
  ...
  const handleBack = () => {
    if (stateFlow == STATEFLOW.Input) navigate({ to: '/main-menu/tutorial' });
    ...
    <ResponsiveScaler scenarioSize={{ width: 1280, height: 720 }} className="flex-1">
      <SafezonePanel className="absolute inset-0 text-black overflow-hidden">
        ...
        >
          <ButtonBack
            onClick={handleBack}
            className="absolute w-14 h-14 left-[7.8rem] cursor-pointer"
          />
          <h2 className="header text-5xl font-semibold">{t('title')}</h2>
        </div>
        ...
        <div className="body" style={dialogStyle}>
          <div className="flex space-x-4 ">
            ...
            <div className="flex space-x-4">
              <Link to="/main-menu/tutorial/input/text">
                <ColumnItem text={t('input_text')} />
              </Link>
              <Link to="/main-menu/tutorial/input/voice">
                <ColumnItem text={t('input_voice')} />
              </Link>
            </div>
          </div>
        </div>
