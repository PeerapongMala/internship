...
export class GCACube extends GameComponent<EngineThreeGameLoopPropsInterface> {
  ...
  Start = ({ context }: EngineThreeGameLoopPropsInterface) => {
    console.log('SceneState #G030101 Main Menu --- Cube');
    const geometry = new THREE.BoxGeometry(1, 1, 1);
    const material = new THREE.MeshBasicMaterial({ color: 0x00ff00 });
    this.cube = new THREE.Mesh(geometry, material);
  };

  Update = ({ timeStamp, deltaTime }: EngineThreeGameLoopPropsInterface) => {
    ...
    if (this.cube) {
      ...
      const offsetX = -4;
      this.cube.position.x = offsetX;
      // this.cube.position.x = Math.sin(timeStamp / 1e3) + offsetX;
      // this.cube.position.y = Math.cos(timeStamp / 1e3) * 2;
    }
  };
