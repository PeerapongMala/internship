...
interface CheckboxProps {
  title: string; // ชื่อของ checkbox หลัก
  options: SubGroup[];
}
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center">
        <input type="checkbox" id={`${title}-checkbox`} onClick={toggleExpand} className="form-checkbox" />
        <label htmlFor={`${title}-checkbox`} className="cursor-pointer ml-2">
          {title}
        </label>
      </div>
          ...
          {options.map((option, index) => (
            <div key={index} className="ml-10">
              <div className="flex items-center">
                ...
                <label className="cursor-pointer ml-2">{option.title}</label> {/* ป4/1 ไม่มี checkbox */}
              </div>
              ...
              {expandedGroups[option.title] && (
                <div className="ml-5 flex items-center gap-5">
                  <div className="flex items-center">
                    <input
                      type="checkbox"
                      id={`${title}-${option.title}-all`}
                      className="form-checkbox"
                      checked={!!checkedGroups[option.title]}
                      onChange={() => handleCheckAll(option.title)}
                    />
                    <label htmlFor={`${title}-${option.title}-all`} className="mt-2 cursor-pointer ml-2">
                      ทั้งหมด
                    </label>
                  </div>
