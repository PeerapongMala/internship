...
const CWModalSelectGroup = ({
  ...
  const [searchField, setSearchField] = useState('study_group_name');

  const searchDropdownOptions = [
    { value: 'study_group_name', label: 'ชื่อกลุ่มเรียน' },
    { value: 'id', label: 'รหัส' },
  ];
  ...
  const columnDefs = useMemo<DataTableColumn<UnlockedGroup>[]>(() => {
    const finalDefs: DataTableColumn<UnlockedGroup>[] = [
      ...
      //     },
      // },
      // { accessor: 'id', title: 'รหัส', width: 80 },
  ...
  const handleAssignGroups = async () => {
    try {
      setLoading(true);
      await ExtraRestAPI.CreateUnlockStudyGroupA26(classId, lessonId, {
        study_group_ids: selectedRecords.map((record) => Number(record.id)),
      });
      showMessage('กลุ่มเรียนถูกกำหนดสำเร็จ', 'success');
      onSuccess?.();
      onClose();
    } catch (error) {
