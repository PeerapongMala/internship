...
const DomainJSX = () => {
  ...
  const subjectDataStore = getUserSubjectData();
  const subjectId = subjectDataStore?.id;
  ...
  useEffect(() => {
    const updateLayout = () => {
      ...
      if (isMobile && window.location.pathname !== '/line/teacher/reward/free') {
        navigate({ to: '/line/teacher/reward/free' });
      }
    };
  ...
  useEffect(() => {
    fetchRewards();
  }, [filterSearch, debouncedFilterSearch, pagination.page, pagination.limit, subjectId]);
  // filter classroom
  useEffect(() => {
        ...
        render: ({ id, status }) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              className={`rounded-md p-2 transition ${isDisabled ? 'cursor-not-allowed text-gray-200' : 'text-black'
                }`}
            >
              <IconCornerUpLeft />
            </button>
            ...
            <div className="flex w-full flex-col gap-2">
              ...
              <div className="flex w-full flex-col items-start gap-2 md:flex-row md:justify-between">
                <div className="flex w-full flex-col gap-2 md:flex-row">
                  <div className="dropdown h-[35px] text-nowrap">
                    <Dropdown
                      placement={'bottom-start'}
                      btnClassName="btn btn-primary dropdown-toggle !font-bold !px-3 gap-1"
                      button={
                        <>
                          Bulk Edit
                          <IconArrowDown />
                  ...
                  <div className="w-[350px] md:w-full flex gap-3">
                    <WCAInputDropdown
                ...
                <CWButton
                  icon={<IconPlus />}
                  title={'รางวัลฟรี'}
                  onClick={() => navigate({ to: '/teacher/reward/free/create' })}
                  className="xs:ml-2 ml-0 text-nowrap"
                />
              </div>
            </div>
