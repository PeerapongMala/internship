...
import { optionYear, optionSubject } from '../local/options';
...
const DomainJSX = () => {
  ...
  const ClickGoCreate = () => {
    navigate({ to: `/teacher/homework/homework/${subjectId}/create` });
  };
  // store login as
  ...
  useEffect(() => {
    ...
    const fetchClasses = async () => {
      try {
        const response = await API.teacherHomework.GetClassList(selectedYear, schoolId);
        if (response.status_code === 200) {
  ...
  const fetchHomeworkList = useCallback(async () => {
    try {
      ...
      const apiParams: Record<string, string | number> = {
        ...
        ...(selectedClass && { room_id: selectedClass }),
      };
  ...
  }, [subjectId, fetchHomeworkList, pagination.page, pagination.limit]);
  ...
  //     selectedRecords.some((selected) => selected.homework_id === record.homework_id)
  //       ? { ...record, status }
  //       : record,
  //   );
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-5 mt-10 flex w-full flex-col gap-3">
        <CWTitleBack
          label={`${curriculum_group_name} / ${subject_name} / ${year_name}`}
          href="../"
        />
      ...
      <WhiteBox>
        <div className="w-full">
          ...
          <div className="mt-5 grid w-full gap-5 xl:grid-cols-3">
            <WCAInputDateFlat
