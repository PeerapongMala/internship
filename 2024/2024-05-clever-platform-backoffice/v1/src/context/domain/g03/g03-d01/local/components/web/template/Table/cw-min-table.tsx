...
const CWTableMin = ({
  ...
  const [allRecords, setAllRecords] = useState<ScoreMin[]>([]);
  const [page, setPage] = useState(1);
  const recordsPerPage = 5;

  useEffect(() => {
    setPage(1);
  }, [academicYear, year, classroom, subject_id, lesson_id]);
  ...
  const fetchMinStudent = async () => {
    if (!classroom || classroom.length === 0 || !subject_id || !lesson_id) {
      return;
    }
    try {
      setFetching(true);
      const res = await API.dashboard.GetA12({
        class_ids: classroom,
  ...
  const columnDefs = useMemo<DataTableColumn<ScoreMin>[]>(() => {
    return [
      {
        accessor: 'index',
        title: '#',
