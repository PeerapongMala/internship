...
const RestAPIDashboard: DashboardRepository = {
  ...
  GetA16: function (
    query: FilterQueryParams,
  ): Promise<PaginationAPIResponse<SubjectFilter>> {
    const url = `${BACKEND_URL}/teacher-dashboard/v1/teacher-subjects`;
    ...
    return fetchWithAuth(url + '?' + params.toString())
      .then((res) => res.json())
      .then((res: PaginationAPIResponse<SubjectFilter>) => {
        return res;
      });
  },
  GetYearFilter: function (
    ...
    const url = `${BACKEND_URL}/teacher-student/v1/years?page=1&limit=-1`;
