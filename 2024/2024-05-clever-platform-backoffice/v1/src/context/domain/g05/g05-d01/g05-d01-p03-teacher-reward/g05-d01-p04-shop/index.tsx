...
const DomainJSX = () => {
  ...
  useEffect(() => {
    const updateLayout = () => {
      ...
      if (isMobile && window.location.pathname !== '/teacher/reward/store/coupon') {
        navigate({ to: '/teacher/reward/store/coupon' });
      }
    };
    updateLayout();
  ...
  const fetchRecords: CWShopRecordsProps['onDataLoad'] = function ({
    setFetching,
    limit,
    page,
    setRecords,
    ...
    if (!userSubjectData.id) {
      return;
    }

    setFetching(true);
    API.store
      .Get(+userSubjectData.id, {
        type: itemType,
        page,
        limit,
    ...
    <div className="w-full">
      ...
      >
        <CWShopRecords
          ...
          userType="teacher"
          className="mantine-mobile-layout"
        />
      </CWMainLayout>
    </div>
  );
