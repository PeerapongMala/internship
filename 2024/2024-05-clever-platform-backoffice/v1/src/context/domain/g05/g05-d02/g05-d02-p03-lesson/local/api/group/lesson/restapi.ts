...
const LessonRestAPI: LessonRepository = {
  GetCourses: async function (classId: string, isParent?: boolean): Promise<Course[]> {
    let url = `${backendUrl}/teacher-lesson/v1/classes/${classId}/curriculum-groups?`;

    if (isParent) {
      url += `is_parent=${isParent}`;
    }
  ...
  GetSubjects: async function (classId: string, isParent?: boolean): Promise<Subject[]> {
    let url = `${backendUrl}/teacher-lesson/v1/classes/${classId}/subjects?`;
  ...
  GetStudentsNoUnlock: async function (
    ...
    const url = `${backendUrl}/teacher-lesson/v1/classes/${classId}/students?page=${page}&limit=${limit}`;
    const response = await fetchWithAuth(url);
    if (!response.ok) {
      throw new Error('Failed to fetch students');
    }
  ...
  GetAllStudents: async function (
    ...
    const url = `${backendUrl}/line-parent/v1/homework/students?page=1&limit=10`;
  ...
  GetAllStudentsListForLesson: async function (
    classId: string,
    page: number,
    limit: number,
    searchField: string,
  ...
  GetClasses: async function (
    ...
    let url = `${backendUrl}/teacher-lesson/v1/classes?page=${page}&limit=${limit}&student_id=${studentId}`;
