...
import { ManageYearStatus, ISubjectGroup, ISubject } from '../local/type';
...
const DomainJSX = () => {
  ...
  const [records, setRecords] = useState<ISubjectGroup[]>([]);
  ...
  const [selectedRecords, setSelectedRecords] = useState<ISubjectGroup[]>([]);

  const [selectedRecord, setSelectedRecord] = useState<ISubjectGroup>();
  ...
  function onArchive(status: 'enabled' | 'disabled', record: ISubjectGroup) {
    API.subjectGroup
      .Update(record.id, {
        ...record,
        status,
  ...
  function onBulkEdit(status: 'enabled' | 'disabled', records: ISubjectGroup[]) {
    API.subjectGroup
      .BulkEdit(
        records.map((r) => ({
          id: r.id,
  ...
  function onUpload(file: File | undefined) {
    if (file) {
      API.subjectGroup.UploadCsv(file, curriculumData.id).then((res) => {
        if (res.status_code === 200 || res.status_code == 201) {
      ...
      <CWTableTemplate
        ...
        table={{
          ...
          columns: [
            ...
            { accessor: 'id', title: 'รหัสกลุ่มวิชา' },
            {
