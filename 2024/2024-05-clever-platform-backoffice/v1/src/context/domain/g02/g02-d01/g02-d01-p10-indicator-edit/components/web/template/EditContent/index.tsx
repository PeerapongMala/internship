...
const EditContent = ({
  ...
  const [data, setData] = useState<IUpdateIndicator>({
    id: undefined,
  ...
  useEffect(() => {
    API.Indicator.GetById(id).then((res) => {
      if (res.status_code === 200) {
        setData((prev) => ({
          ...
          transcript_name: res.data.transcript_name,
    ...
    >
      ...
      <div className="mt-10 flex w-full gap-5">
        <div className="grid h-auto w-[75%] grid-cols-6 gap-5 bg-white p-5">
            ...
            <CWSelectValue
              ...
              onChange={(selectedOption) => {
                if (selectedOption) {
                  setData((prev) => ({
                    ...prev,
                    learning_content_id: selectedOption.value,
                  }));
                }
              }}
            />
          ...
          <div className="col-span-3 w-full">
            <CWInput
              value={data.transcript_name}
              onChange={(e) =>
                setData((prev) => ({ ...prev, transcript_name: e.target.value }))
              }
              required={true}
              label={'ชื่อย่อบน ปพ.'}
              className="col-span-2"
