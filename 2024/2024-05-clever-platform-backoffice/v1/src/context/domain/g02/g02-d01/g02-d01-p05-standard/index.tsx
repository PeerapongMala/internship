...
import StandardTable from './components/web/organism/StandardTable';
import {
  Curriculum,
  Learning,
  Standard,
...
const DomainJSX = () => {
  ...
  const [selectedRecords, setSelectedRecords] = useState<Standard[]>([]);
  ...
  const confirmArchive = () => {
    ...
    modalRecall.open();
    API.Standard.Update(selectArchive.id as number, {
      ...selectArchive,
    })
      .then((res) => {
        if (res.status_code === 200) {
  ...
  const handleBulkEdit = (status: LearningStatus) => {
    ...
    setFetching(true);
    API.Standard.BulkEdit({ bulk_edit_list: updatedRecords as BulkEditItem[] })
      .then((res) => {
        console.log(res.message);
        if (res.status_code === 200) {
  ...
  const columnDefs = useMemo<DataTableColumn<Standard>[]>(() => {
    return [
      {
        accessor: 'index',
        title: '#',
      ...
      { accessor: 'name', title: 'มาตรฐาน' },
      { accessor: 'content_name', title: 'สาระ' },
      {
        accessor: 'updated_at',
        title: 'แก้ไขล่าสุด',
        render: ({ updated_at }) => {
      ...
      },
      {
        ...
        title: 'แก้ไข',
        render: ({ id }) => (
          <Link to="./$standardId/edit" params={{ standardId: id }}>
            <IconPen />
          </Link>
        ),
      },
