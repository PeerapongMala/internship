...
const SubmitOnMultipleChoices = async ({
  academicLevelId,
  academicLevel,
  timerType,
  timerTime,
  ...
  console.log('SubmitOnMultipleChoices', inputAnswerList);
  ...
  // const useSoundDescriptionOnly =
  //   optionsAnswerSelect.find((item) => item.value === selectedAnswerType)?.label ===
  //   'speech'
  //     ? 'true'
  //     : 'false';
  ...
  // formData.append("correct_choice_amount", selectedAnswerCount);
  // formData.append("max_point", selectedAnswerScore);
  ...
  if (selectedQuestion?.id) {
    ...
    return API.academicLevel
      .UpdateG02D05A09(selectedQuestion?.id.toString(), formData)
      .then((res) => {
    ...
    return API.academicLevel.CreateG02D05A08(formData).then((res) => {
