...
const rowData: RowData[] = [
  ...
  },
  {
    ...
    topic: 'คำตอบ',
    language_th:
      'กานดาได้รับเงินเดือนเดือนละ 52,680 บาท ในเดือนนี้เขาได้รับเงินเดือนรวม กับเงินโบนัส 111,350 บาท “กานดาได้รับเงินโบนัสกี่บาท” เขียนเป็นประโยคสัญลักษณ์ได้อย่างไร?',
    language_th_sound: '',
    language_en: '',
    language_en_ai: '',
    language_en_sound: '',
...
const getComponentSound = (
  record: Record<string, unknown>,
  index: number,
  language_sound: string,
): ReactNode => {
  if ((record as RowData)[language_sound] === '') {
    return (
      <div className="flex items-center gap-4">
        <div className="h-4 w-4 rounded-full bg-danger" />
        <button type="button" className="btn btn-primary h-8 w-8 rounded-full !p-0">
          <IconPlus className="h-5 w-5" duotone={false} />
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <button type="button" className="btn btn-primary h-8 w-8 rounded-full !p-0">
        <IconVolumeLoud className="h-4 w-4" />
      </button>
      <button type="button" className="btn btn-primary h-8 w-8 rounded-full !p-0">
        <IconTrashLines duotone={false} className="h-5 w-5" />
      </button>
    </div>
  );
};
...
const rowColumns: DataTableColumn[] = [
  ...
  },
  {
    ...
    title: 'เสียงภาษาไทย',
    render: (record: Record<string, unknown>, index: number) =>
      getComponentSound(record, index, 'language_th_sound'),
  },
  {
    accessor: 'language_en',
    title: 'ภาษาอังกฤษ',
    width: 450,
    render: (record: Record<string, unknown>, index: number) => (
  ...
  },
  {
    ...
    render: (record: Record<string, unknown>, index: number) =>
      getComponentSound(record, index, 'language_en_sound'),
  },
  {
  ...
  },
  {
    ...
    title: 'เสียงภาษาจีน',
    render: (record: Record<string, unknown>, index: number) =>
      getComponentSound(record, index, 'language_cn_sound'),
  },
];
