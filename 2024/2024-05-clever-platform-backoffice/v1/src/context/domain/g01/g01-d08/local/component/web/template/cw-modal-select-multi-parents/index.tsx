...
// v1/src/context/global/component/web/cw-modal/Modal.tsx
...
interface CWModalSelectMultiParentsProps<T extends WithUserId> {
  open: boolean;
  onClose: () => void;
...
const CWModalSelectMultiParents = <T extends WithUserId>({
  open,
  onClose,
  setSelectItems,
  renderRow,
  ...
  familyId,
  setRefetch,
}: CWModalSelectMultiParentsProps<T>) => {
    ...
    <Modal open={open} onClose={onClose} title={title} className="h-auto w-3/4">
      <div className="flex h-auto w-full flex-col gap-4">
              ...
              {parentsList.map((item: any, index: number) => (
                <div className="flex items-center p-2" key={index}>
                  <input
                    ...
                    onChange={() => handleSelectItem(item as T)} // เมื่อเปลี่ยนสถานะ checkbox
                    className="form-checkbox mr-3 flex items-center justify-center" // เพิ่มการจัดการระยะห่าง
                  />
                  <div
                    className={`flex w-full flex-row items-center justify-between rounded-md border px-4 py-2 ${internalSelectedItems.some(
                      (selectedItem) =>
                        JSON.stringify(selectedItem) === JSON.stringify(item),
                    )
        ...
        <div className="flex w-full justify-between">
          <CWButton outline title="ยกเลิก" onClick={onClose} className="w-[150px]" />
          <CWButton
...
export default CWModalSelectMultiParents;
