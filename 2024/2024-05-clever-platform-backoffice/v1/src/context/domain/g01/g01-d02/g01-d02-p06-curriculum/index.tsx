...
import CWMHeader from '@component/web/molecule/cw-m-header';
...
const DomainJSX = () => {
  ...
  const [records, setRecords] = useState<CurriculumGroup[]>([]);
  ...
  const [selectedRecords, setSelectedRecords] = useState<CurriculumGroup[]>([]);

  const [selectedRecord, setSelectedRecord] = useState<CurriculumGroup>();
    ...
    API.curriculumGroup
      ...
      .then((res) => {
        if (res instanceof Blob) {
          downloadCSV(res, 'curriculums.csv');
        } else {
  ...
  function onUpload(file: File | undefined) {
    if (file) {
      API.curriculumGroup.UploadCSV(file).then((res) => {
  ...
  function onArchive(status: 'enabled' | 'disabled', record: CurriculumGroup) {
    API.curriculumGroup
      .Update(record.id, {
        ...record,
        status,
  ...
  function onBulkEdit(status: 'enabled' | 'disabled', records: CurriculumGroup[]) {
    API.curriculumGroup
      .BulkEdit(
        records.map((record) => ({
          curriculum_group_id: record.id,
          status,
        })),
      )
      .then((res) => {
  ...
  useEffect(() => {
    fetchRecords();
  }, [page, limit, searchText, statusTabIndex, search]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div>
        <div className="text-2xl font-bold">สังกัดวิชา</div>
      </div>
      ...
      <CWTableTemplate
        header={{
          ...
          btnIcon: <IconPlus />,
          btnLabel: 'เพิ่มสังกัดวิชา',
          onBtnClick() {
            navigate({ to: '/admin/curriculum/create' });
          },
          ...
          searchDropdownOptions: [
            {
              label: 'รหัสสังกัดวิชา',
              value: 'id',
            },
            {
              label: 'ชื่อสังกัดวิชา',
              value: 'name',
            },
            {
              label: 'ชื่อย่อ',
            ...
            },
            {
              ...
              render(record, index) {
                return (
                  <button
                    onClick={() => {
                      navigate({ to: `/admin/curriculum/${record.id}` });
                    }}
                  >
                    <IconSearch />
                  </button>
            ...
            <CWButton
              ...
              onClick={() => {
                if (selectedRecord) {
                  ...
                  closeModal();
                } else showMessage('กรุณาเลือกหลักสูตร', 'warning');
              }}
            />
          </div>
        </div>
