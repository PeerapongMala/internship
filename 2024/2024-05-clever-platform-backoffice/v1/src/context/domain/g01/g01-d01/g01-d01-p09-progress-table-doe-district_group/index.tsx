...
const ProgressTableDoeDistrictGroup = () => {
  const navigate = useNavigate();
  const { startDate, endDate, setStartDate, setEndDate, resetDates } =
    useDateRangeStore();
  const { district_group } = useParams({ strict: false });
  // Sidebar
  useEffect((): void => {
  ...
  const [scope, setScope] = useState<string>('district');
  const [parent_scope, setParentScope] = useState<string>(district_group);
...
export default ProgressTableDoeDistrictGroup;
