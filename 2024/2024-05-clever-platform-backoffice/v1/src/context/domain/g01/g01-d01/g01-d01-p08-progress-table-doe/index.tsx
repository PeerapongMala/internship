...
const ProgressTableDOE = () => {
  ...
  const [selectedTab, setSelectedTab] = useState<string>('doe');
  ...
  const [scope, setScope] = useState<string>('district-zone');
  const [parent_scope, setParentScope] = useState<string>('');
  const [report_type, setReportType] = useState<string>('doe');
      ...
      render: (record: IRecords) => (
        <div className="flex w-full justify-center">
          <button
            type="button"
            onClick={() => {
              navigate({ to: `${location.pathname}/${record.scope}` });
            }}
          >
  ...
  return (
    <div className="flex flex-col gap-5">
      <div hidden={!isFirstLoad} className="relative h-screen w-full">
        <CWALoadingOverlay visible={isFirstLoad} />
      </div>

      <div hidden={isFirstLoad} className="flex flex-col gap-5">
        <CWBreadcrumbs
          links={[
            {
              label: 'รายงาน',
          ...
          <div className="flex flex-col gap-2">
            <div className="rounded-md p-2.5">
              <div className="flex gap-4">
                <p className="text-[26px] font-bold">{'Report สนศ. กทม.'}</p>
              </div>
            </div>
...
export default ProgressTableDOE;
