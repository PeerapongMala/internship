...
const ProgressTableOBECArea = () => {
  const navigate = useNavigate();
  const { startDate, endDate, setStartDate, setEndDate, resetDates } =
    useDateRangeStore();
  const { district_zone, area_office } = useParams({ strict: false });
  ...
  const [parent_scope, setParentScope] = useState<string>(area_office);
          ...
          <div className="rounded-md p-2.5">
            <div className="flex gap-4">
              ...
              <p className="text-[26px] font-bold">
                {'Report สพฐ.'} / {district_zone} / {area_office}
              </p>
            </div>
          </div>
...
export default ProgressTableOBECArea;
