...
const ProgressTableDoeDistrict = () => {
  const navigate = useNavigate();
  const { startDate, endDate, setStartDate, setEndDate, resetDates } =
    useDateRangeStore();
  const { district_group, district } = useParams({ strict: false });
  // Sidebar
  useEffect((): void => {
  ...
  const [scope, setScope] = useState<string>('doe-school');
  const [parent_scope, setParentScope] = useState<string>(district);
          ...
          <div className="rounded-md p-2.5">
            <div className="flex gap-4">
              ...
              <p className="text-[26px] font-bold">
                {'Report สนศ. กทม.'} / {district_group} / {district}
              </p>
            </div>
          </div>
...
export default ProgressTableDoeDistrict;
