...
const ProgressTableOBECDistrict = () => {
  const navigate = useNavigate();
  const { startDate, endDate, setStartDate, setEndDate, resetDates } =
    useDateRangeStore();
  const { district_zone } = useParams({ strict: false });
  ...
  const [scope, setScope] = useState<string>('area-office');
  const [parent_scope, setParentScope] = useState<string>(district_zone);
...
export default ProgressTableOBECDistrict;
