...
const ProgressTableDoeSchool = () => {
  const navigate = useNavigate();
  const { startDate, endDate, setStartDate, setEndDate, resetDates } =
    useDateRangeStore();
  const { district_group, district, school } = useParams({ strict: false });
  // Sidebar
  useEffect((): void => {
          ...
          <div className="rounded-md p-2.5">
            <div className="flex gap-4">
              ...
              <p className="text-[26px] font-bold">
                {'Report สนศ. กทม.'} / {district_group} / {district} / {school}
              </p>
            </div>
          </div>
...
export default ProgressTableDoeSchool;
