...
interface SchoolDataTableProps {
  ...
  colunms?: ColumnHeader[];
}

const DataGridWC: React.FC<SchoolDataTableProps> = ({ initialRecords, colunms }) => {
  ...
  return (
    <div className="flex flex-col gap-5 overflow-x-auto">
      <table className="min-w-full whitespace-nowrap bg-white dark:bg-black">
        <thead>
          <tr>
            {colunms?.map((column, index) => <th key={index}>{column.header}</th>)}
            <th>#</th>
            <th>ปีการศึกษา</th>
            <th>สังกัดวิชา</th>
            <th>วิชา</th>
            ...
            {[...Array(totalPages)].map((_, index) => (
              <li key={index}>
                <button
                  ...
                  className={`flex justify-center rounded-full px-3.5 py-2 font-semibold transition ${
                    page === index + 1
