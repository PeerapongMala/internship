...
const TableRow = ({ children, style = {} }: any) => (
    <View style={[styles.tableRow, { marginLeft: '5%' }, style]}>{children}</View>
);
...
const CWTemplate1PDF = ({
    ...
    const headerBgStyle = {
        backgroundColor: addOpacityToHex(colourSetting?.position3 || '', 0.5),
    };
        ...
        <Document>
            <Page size="A4" style={[styles.page, pageBgStyle]}>
                <View style={styles.container}>
                    {/* Header */}
                    <View style={[styles.headerContainer, headerBgStyle]}>
                    ...
                    <View style={styles.studentInfoContainer}>
                        ...
                        <View style={styles.studentInfoRight}>
                            <Text>
                                <Text style={{ fontWeight: 'bold' }}>เลขประจำตัว</Text> {studentData?.idNo}
                            </Text>
                            <Text>
                    ...
                    <View style={[styles.table, { borderBottomWidth: 0, position: 'relative' }]}>
                        ...
                        <TableRow style={headerBgStyle}>
                            <TableCell width="12%" style={styles.tableHeader} textAlign="center">รหัสวิชา</TableCell>
                            <TableCell width="22%" style={styles.tableHeader} textAlign="center">รายวิชา</TableCell>
                        ...
                        {defaultTo(studentDetail?.dataJson?.subject, []).map((subject, index) => (
                            <TableRow key={index}>
                                <TableCell width="12%">{subject.subjectCode || '-'}</TableCell>
                                <TableCell width="22%">{subject.subjectName || '-'}</TableCell>
                                <TableCell width="10%" textAlign="center">{subject.hours || '-'}</TableCell>
                                <TableCell width="10%" textAlign="center">{subject.totalScore || '-'}</TableCell>
                                ...
                                <TableCell width="12%" textAlign="center">{subject.note || ''}</TableCell>
                            </TableRow>
                        ))}
                        ...
                        {/* Total row */}
                        <TableRow>
                            <TableCell width="34%" textAlign="center" style={{ fontWeight: 'bold' }}>
                                รวม
                            </TableCell>
                            <TableCell width="10%" textAlign="center" style={{ fontWeight: 'bold' }}>
                                {summary?.hours || '-'}
                            </TableCell>
                            <TableCell width="10%" textAlign="center" style={{ fontWeight: 'bold' }}>
                                {summary?.totalScore || '-'}
                    ...
                    <View style={styles.contentContainer}>
                        ...
                        <View style={styles.leftColumn}>
                            ...
                            <View style={[styles.summaryTable, summaryBgStyle]}>
                                <SummaryRow>
                                    <SummaryCell width="70%">คะแนนคิดเป็นร้อยละ</SummaryCell>
                                    <SummaryCell width="30%" textAlign="center">
                                        {grades?.scorePercentage ? Math.round(Number(grades.scorePercentage)) : 0}
                                    </SummaryCell>
                                </SummaryRow>
                                <SummaryRow>
                                    <SummaryCell width="70%">คะแนนรวมได้ลำดับที่</SummaryCell>
                                    <SummaryCell width="30%" textAlign="center">
                                        {grades?.totalScoreRank || '-'}
                                    </SummaryCell>
                                </SummaryRow>
                                <SummaryRow>
                                    <SummaryCell width="70%">ผลการเรียนเฉลี่ย</SummaryCell>
                                    <SummaryCell width="30%" textAlign="center">
                                        {grades?.avgLearnScore || 0}
                                    </SummaryCell>
                                </SummaryRow>
                                <SummaryRow>
                                    <SummaryCell width="70%">ผลการเรียนเฉลี่ยได้ลำดับที่</SummaryCell>
                                    <SummaryCell width="30%" textAlign="center">
                                        {grades?.avgLearnRank || '-'}
                                    </SummaryCell>
                                </SummaryRow>
                            ...
                            {/* Activity Evaluation */}
                            <View style={[styles.summaryTable, summaryBgStyle, { marginTop: 5 }]}>
                                <SummaryRow>
                                ...
                                {activityData?.map((activity) => (
                                    activity.studentIndicatorData.map((indicator, idx) => (
                                        <SummaryRow key={`activity-${activity.evaluationStudentId}-${idx}`}>
                                            <SummaryCell width="70%">{indicator.indicatorGeneralName}</SummaryCell>
                                            <SummaryCell width="30%" textAlign="center">
