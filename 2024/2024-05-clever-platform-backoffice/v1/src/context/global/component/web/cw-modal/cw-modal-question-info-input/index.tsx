...
interface CWModalQuestionInfoInputProps extends ModalProps {
  title: string;
  open: boolean;
  onClose: () => void;
}

const CWModalQuestionInfoInput = ({
  title,
  open,
  onClose,
}: CWModalQuestionInfoInputProps) => {
  // Mock data for multiple questions
  const questions = [
    {
      id: 1,
      question: 'Ann: Good Morning, {Ans1} do you do?',
      duration_seconds: 8,
      correct_answer: ['How'],
      answer: 'How',
    },
            ...
            <div key={question.id} className="flex flex-col gap-2">
              ...
              >
                ...
                <span className="text-right text-sm text-neutral-900">
                  {question.correct_answer.includes(question.answer) ? 'ถูก' : 'ผิด'}
                </span>
              </div>
                    ...
                    <div className="flex flex-col gap-2">
                      {questions.map((q, i) => (
                        <div className="mb-3 flex flex-col gap-2 text-sm text-neutral-900">
                          <div className="font-bold">โจทย์ #{i + 1}</div>
                          <div className="break-words">{q.question}</div>
                          <div className="font-bold">ตอบ</div>
                          <div className="break-words">{q.answer}</div>
                        </div>
                      ))}
                    </div>
                  </div>
...
export default CWModalQuestionInfoInput;
