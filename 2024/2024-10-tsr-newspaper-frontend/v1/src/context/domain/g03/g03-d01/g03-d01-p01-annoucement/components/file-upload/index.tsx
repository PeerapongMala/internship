...
import ImageCoverList from '../image-cover-list';
import { FileUploadState, maxCountImage } from '../annoucement';
...
const FileUpload: React.FC<FileUploadProps> = ({ imageState, setImageState }) => {
  ...
  useEffect(() => {
    setIsDisable(imageState.imageCoverList.length === maxCountImage);
  }, [imageState]);

  const handleCropAndSetPreview = (file: File) => {
    setImageState((prev) => {
      const isMaxCountImage = prev.imageCoverList.length === maxCountImage;

      if (isMaxCountImage) {
        return prev;
      }

      const newOrder = prev.imageCoverList.length + 1;
      const newImage = {
        file: file,
      ...
      return {
        ...prev,
        imageCoverList: [...prev.imageCoverList, newImage],
  ...
  const handleInitialDrop = (e: DragEvent<HTMLDivElement>) => {
    ...
    if (droppedFile) {
      setPrevFiles({
        file: droppedFile,
        display_order: imageState.imageCoverList.length + 1,
      });
  ...
  const handleSortDrop = (_e: DragEvent<HTMLDivElement>, dropIndex: number) => {
    ...
    setImageState((prev) => {
      const newFiles = [...prev.imageCoverList];
  ...
  const removeFile = (index: number) => {
    const fileToRemove = imageState.imageCoverList[index];
    ...
    setImageState((prev) => {
      const filteredList = prev.imageCoverList.filter((_, idx) => idx !== index);
  ...
  return (
    <div>
      {imageState.imageCoverList.length > 0 && (
        <ImageCoverList
          imageCoverList={imageState.imageCoverList}
      ...
      <DropZone
        ...
        resolution="1,440 x 632 px | Safe zone 671 x 632 px"
      />
