...
import CalendarDarkIcon from '../icon/celendar-dark.svg';
...
const DateRangeSelector: React.FC<DatePickerProps> = (props) => {
  const { value, label, onChange, minDate, maxDate, isOpen, onOpenChange } = props;
  ...
  const isCalendarOpen = isOpen ?? internalIsOpen;
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      // เช็คว่าคลิกที่ input หรือไม่
      const isClickInput = calendarRef.current.contains(target);

      // เช็คว่าคลิกที่ calendar content หรือไม่
      const isClickCalendar = calendarContentRef.current?.contains(target);

      // ถ้าไม่ได้คลิกทั้ง input และ calendar ให้ปิด
      if (!isClickInput && !isClickCalendar) {
  ...
  const toggleCalendar = (newValue: boolean) => {
    if (onOpenChange) {
  ...
  const handleDateSelect = (day: number) => {
    ...
    onChange?.(utcDate); // ส่งค่าแบบ UTC
    toggleCalendar(false);
  };
  ...
  const handleSelectToday = () => {
    ...
    onChange?.(utcToday); // ส่งค่าแบบ UTC
      ...
      {isCalendarOpen && (
        <>
          ...
          <div
            ...
            className={`fixed bg-white rounded-lg shadow-lg px-4 py-1 z-50 w-[280px] dark:bg-[#414141] border border-[#D2D2D2]
