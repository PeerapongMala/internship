...
type QuestionGroupUpdateRequest struct {
	...
	GroupAmount                int    `form:"group_amount"`
...
func (api *APIStruct) QuestionGroupUpdate(context *fiber.Ctx) error {
	...
	request, err := helper.ParseAndValidateRequest(context, &QuestionGroupUpdateRequest{})
	if err != nil {
		...
		if err != nil {
			if err.Error() == "there is no uploaded file associated with the given key" {
				...
				if value == "null" {
					...
					log.Println(len(request.ChoiceImages))
					continue
				} else {
					break
				}
	...
	log.Println("length of choice images: ", len(request.ChoiceImages))
	for _, image := range request.ChoiceImages {
		if image != nil {
	...
	questionGroupUpdateOutput, err := api.Service.QuestionGroupUpdate(&QuestionGroupUpdateInput{
		Roles:                      roles,
		SubjectId:                  subjectId,
	...
	return context.Status(http.StatusOK).JSON(QuestionGroupUpdateResponse{
		StatusCode: http.StatusOK,
		Data:       []interface{}{questionGroupUpdateOutput.QuestionGroup},
...
type QuestionGroupUpdateOutput struct {
	QuestionGroup interface{}
}

func (service *serviceStruct) QuestionGroupUpdate(in *QuestionGroupUpdateInput) (*QuestionGroupUpdateOutput, error) {
	...
	err = copier.Copy(&questionEntity, in.QuestionGroupUpdateRequest)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, err
	}
	...
	err = copier.Copy(&questionGroupEntity, in.QuestionGroupUpdateRequest)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, err
	}
	questionGroupEntity.QuestionId = in.QuestionId

	_, err = service.academicLevelStorage.QuestionGroupUpdate(tx, &questionGroupEntity)
	if err != nil {
		return nil, err
	}
	...
	err = service.UpdateQuestionGroupGroups(&UpdateQuestionGroupGroupsInput{
		Tx:         tx,
		QuestionId: in.QuestionId,
		Groups:     in.Groups,
	})
	...
	groups, err := service.academicLevelStorage.QuestionGroupGroupCaseListByQuestion(tx, question.Id)
	if err != nil {
		return nil, err
	}

	updateQuestionGroupChoicesOutput, err := service.UpdateQuestionGroupChoices(&UpdateQuestionGroupChoicesInput{
		Tx:           tx,
	...
	keysToDelete = append(keysToDelete, updateQuestionGroupChoicesOutput.KeysToDelete...)

	for key, value := range updateQuestionGroupChoicesOutput.KeysToAdd {
	...
	getFullQuestionGroupOutput, err := service.GetFullQuestionGroup(&GetFullQuestionGroupInput{
	...
	return &QuestionGroupUpdateOutput{
		getFullQuestionGroupOutput.FullQuestionGroup,
	}, nil
}
