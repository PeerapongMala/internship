...
func (postgresRepository *postgresRepository) LessonCaseGetCurriculumGroupId(lessonId int) (*int, error) {
	query := `
		SELECT
	...
	err := postgresRepository.Database.QueryRowx(query, lessonId).Scan(&curriculumGroupId)
	if err != nil {
