...
func (api *APIStruct) SubCriteriaTopicUpdate(context *fiber.Ctx) error {
	var body constant.SubCriteriaTopicsUpdateRequest

	TopicIdStr := context.Params("topicId")
	TopicId, err := strconv.Atoi(TopicIdStr)
	if err != nil {
		return context.Status(fiber.StatusBadRequest).JSON(constant.StatusResponse{
			StatusCode: fiber.StatusBadRequest,
			Message:    "Topic Id should be a valid integer",
		})
	}
	...
	err = api.Service.SubCriteriaTopicUpdate(body)
	if err != nil {
		...
		if err.Error() == "Topic id is not exist" {
			return context.Status(fiber.StatusNotFound).JSON(constant.StatusResponse{
				StatusCode: fiber.StatusNotFound,
				Message:    "Topic Id does not exist",
			})
...
func (service *serviceStruct) SubCriteriaTopicUpdate(c constant.SubCriteriaTopicsUpdateRequest) error {
