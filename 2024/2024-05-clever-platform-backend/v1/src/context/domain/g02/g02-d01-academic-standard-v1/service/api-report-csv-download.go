...
func (api *APIStruct) ReportCsvDowload(context *fiber.Ctx) error {
	...
	fileBytes, err := api.Service.ReportCsvDowload(check, SubCriteriaId, request.StartDate, request.EndDate, pagination)
	if err != nil {
...
func (service *serviceStruct) ReportCsvDowload(check constant.CheckAdmin, SubcriteriaId int, StartDate string, EndDate string, pagination *helper.Pagination) ([]byte, error) {
	...
	response, _, err := service.repositoryStorage.GetReport(SubcriteriaId, pagination)
	if err != nil {
		return nil, err
	}
	...
	Data := [][]string{constant.ReportCSVHeader}
	for i, item := range response {
		Data = append(Data, []string{
			strconv.Itoa(i + 1),
			strconv.Itoa(item.IndicatorId),
			item.IndicatorShortName,
			item.SeedYearName,
			item.ContentName,
			item.CriteriaName,
