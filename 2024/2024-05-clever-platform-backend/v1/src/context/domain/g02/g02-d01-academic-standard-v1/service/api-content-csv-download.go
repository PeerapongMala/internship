...
type DownloadContentCSVRequest struct {
	constant.DownloadCSVRequest
	CurriculumGroupId int `query:"curriculum_group_id"`
}

func (api *APIStruct) ContentDownloadCSV(context *fiber.Ctx) error {

	request := &DownloadContentCSVRequest{}
	...
	fileBytes, err := api.Service.ContentDownloadCSV(request, filter)
	if err != nil {
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}
...
func (service *serviceStruct) ContentDownloadCSV(req *DownloadContentCSVRequest, filter constant.ContentFilter) ([]byte, error) {

	response, _, err := service.repositoryStorage.GetContent(req.CurriculumGroupId, filter, &constant.PaginationDefault)
	if err != nil {
		return nil, err
	}
	...
	//prepare csv data
	csvData := [][]string{constant.ContentCSVHeader}
