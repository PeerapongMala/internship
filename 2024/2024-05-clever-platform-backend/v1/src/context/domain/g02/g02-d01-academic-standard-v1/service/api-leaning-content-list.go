...
func (api *APIStruct) GetLearningContent(context *fiber.Ctx) error {
	...
	response, totalCount, err := api.Service.GetLearningContent(req, filter, pagination)
	if err != nil {
...
func (service *serviceStruct) GetLearningContent(r constant.ListRequest, filter constant.LearningContentFilter, pagination *helper.Pagination) ([]constant.LearningContentResponse, int, error) {
	check := false
	...
	response, totalCount, err := service.repositoryStorage.GetLearningContent(r.CurriculumGroupId, filter, pagination)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, 0, err
	}
