package postgres

import (
	"fmt"
	"github.com/jmoiron/sqlx"
	"time"

	"github.com/ZettaMerge/2024-05-clever-platform-backend/src/context/domain/g02/g02-d01-academic-standard-v1/constant"
)

func (postgresRepository *postgresRepository) IndicatorUpdate(c constant.IndicatorsUpdateRequest, txs ...*sqlx.Tx) error {
	var QueryMethod sqlx.Ext
	if len(txs) > 0 {
		QueryMethod = txs[0]
	} else {
		QueryMethod = postgresRepository.Database
	}
	query := `UPDATE "curriculum_group"."indicator"
	SET learning_content_id = $1,
	name  = $2,
	short_name  = $3,
	transcript_name  = $4,
	status  = $5,
	updated_at  = $6,
	updated_by = $7
	WHERE id = $8
	`

	result, err := QueryMethod.Exec(query,
		c.LearningContentId,
		c.Name,
		c.ShortName,
		c.TranscriptName,
		c.Status,
		time.Now().UTC(),
		c.UpdatedBy,
		c.Id,
	)
	if err != nil {
		return err
	}
	RowsAffected, err := result.RowsAffected()

	if err != nil {
		return err
	}
	if RowsAffected == 0 {
		return fmt.Errorf("indicators id is not exist")
	}
	return nil
}
