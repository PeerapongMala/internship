...
type DownloadCriteriaCSVRequest struct {
	constant.DownloadCSVRequest
	CurriculumGroupId int `query:"curriculum_group_id"`
}

func (api *APIStruct) CriteriaDownloadCSV(context *fiber.Ctx) error {

	request := &DownloadCriteriaCSVRequest{}
	...
	fileBytes, err := api.Service.CriteriaDownloadCSV(request, filter)
	if err != nil {
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}
...
func (service *serviceStruct) CriteriaDownloadCSV(req *DownloadCriteriaCSVRequest, filter constant.CriteriaFilter) ([]byte, error) {

	response, _, err := service.repositoryStorage.GetCriteria(req.CurriculumGroupId, filter, &constant.PaginationDefault)
	if err != nil {
		return nil, err
	}
	...
	//prepare csv data
	csvData := [][]string{constant.CriteriaCSVHeader}
