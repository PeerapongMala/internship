...
func (api *APIStruct) LearningContentCsvDowload(context *fiber.Ctx) error {
	...
	fileBytes, err := api.Service.LearningContentCsvDowload(check, request.CurriculumGroupId, request.StartDate, request.EndDate, filter, pagination)
	if err != nil {
...
func (service *serviceStruct) LearningContentCsvDowload(check constant.CheckAdmin, curriculumGroupId int, StartDate string, EndDate string, filter constant.LearningContentFilter, pagination *helper.Pagination) ([]byte, error) {
	valid := false
	...
	response, _, err := service.repositoryStorage.GetLearningContent(curriculumGroupId, filter, pagination)
	if err != nil {
		return nil, err
	}
	...
	Data := [][]string{constant.LearningContentCSVHeader}
	for i, item := range response {
		Data = append(Data, []string{
			strconv.Itoa(i + 1),
			strconv.Itoa(item.Id),
			item.Name,
			item.CriteriaShortName,
			item.CriteriaName,
			item.Status,
		})
	}
