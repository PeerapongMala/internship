...
func (api *APIStruct) GetCriteria(context *fiber.Ctx) error {
	...
	response, totalCount, err := api.Service.GetCriteria(req, filter, pagination)
	if err != nil {
...
func (service *serviceStruct) GetCriteria(r constant.ListRequest, filter constant.CriteriaFilter, pagination *helper.Pagination) ([]constant.CriteriaResponse, int, error) {
	check := false
	...
	response, totalCount, err := service.repositoryStorage.GetCriteria(r.CurriculumGroupId, filter, pagination)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, 0, err
	}
