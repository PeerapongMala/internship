...
func (api *APIStruct) LearningContentUpdate(context *fiber.Ctx) error {
	var body constant.LearningContentUpdateRequest

	LearningContentIdStr := context.Params("learningContentId")
	LearningContentId, err := strconv.Atoi(LearningContentIdStr)
	if err != nil {
		return context.Status(fiber.StatusBadRequest).JSON(constant.StatusResponse{
			StatusCode: fiber.StatusBadRequest,
			Message:    "Learning Content Id should be a valid intger",
		})
	}
	body.Id = LearningContentId
	...
	err = api.Service.LearningContentUpdate(body)
	if err != nil {
		...
		if err.Error() == "Learning Id is not exist" {
			return context.Status(fiber.StatusNotFound).JSON(constant.StatusResponse{
				StatusCode: fiber.StatusNotFound,
				Message:    "Learning Id is not exist",
			})
		}
		return context.Status(fiber.StatusInternalServerError).JSON(constant.StatusResponse{
			StatusCode: fiber.StatusInternalServerError,
			Message:    "Cannot Update Learning Content",
		})
	}
	return context.Status(fiber.StatusOK).JSON(constant.StatusResponse{
		StatusCode: fiber.StatusOK,
		Message:    "Learning Content Update",
	})
}
func (service *serviceStruct) LearningContentUpdate(c constant.LearningContentUpdateRequest) error {
	curriculumGroupId, err := service.repositoryStorage.GetByCriteriaId(c.CriteriaId)
	if err != nil {
		if err.Error() == "criteria id is not exist" {
	...
	if err != nil {
		if err.Error() == "Learning Id is not exist" {
			return fmt.Errorf("Learning Id is not exist")
		}
		log.Printf("%+v", errors.WithStack(err))
		return err
