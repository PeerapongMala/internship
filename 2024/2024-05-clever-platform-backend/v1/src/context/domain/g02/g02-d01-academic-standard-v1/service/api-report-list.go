...
func (api *APIStruct) GetReport(context *fiber.Ctx) error {
	...
	response, totalCount, err := api.Service.GetReport(SubCriteriaId, req, pagination)
	if err != nil {
...
func (service *serviceStruct) GetReport(SubcriteriaId int, r constant.ListRequest, pagination *helper.Pagination) ([]constant.ReportResponse, int, error) {
	...
	response, totalCount, err := service.repositoryStorage.GetReport(SubcriteriaId, pagination)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, 0, err
	}
