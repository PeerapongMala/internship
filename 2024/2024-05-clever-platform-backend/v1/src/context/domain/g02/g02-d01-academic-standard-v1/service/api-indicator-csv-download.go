...
func (api *APIStruct) IndicatorCsvDowload(context *fiber.Ctx) error {
	...
	fileBytes, err := api.Service.IndicatorCsvDowload(check, request.CurriculumGroupId, request.StartDate, request.EndDate, filter, pagination)
	if err != nil {
...
func (service *serviceStruct) IndicatorCsvDowload(check constant.CheckAdmin, curriculumGroupId int, StartDate string, EndDate string, filter constant.IndicatorsFilter, pagination *helper.Pagination) ([]byte, error) {
	valid := false
	...
	response, _, err := service.repositoryStorage.GetIndicators(curriculumGroupId, filter, pagination)
	if err != nil {
		return nil, err
	}
	...
	Data := [][]string{constant.IndicatorCSVHeader}
