...
func (postgresRepository *postgresRepository) TeacherCaseListCurriculumGroup(teacherId string, classId int, pagination *helper.Pagination, isParent bool) ([]constant.CurriculumGroupEntity, error) {
	query := `
...
`
	...
	if pagination != nil {
		...
		query += fmt.Sprintf(` ORDER BY "cg"."id" OFFSET $%d LIMIT $%d`, argsIndex, argsIndex+1)
		argsIndex += 2
