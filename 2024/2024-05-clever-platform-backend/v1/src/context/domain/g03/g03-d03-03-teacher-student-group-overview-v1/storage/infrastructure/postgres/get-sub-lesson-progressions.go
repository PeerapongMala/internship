...
func (postgresRepository *postgresRepository) GetSubLessonProgressions(
	classId int,
	subjectId int,
	lessonId int,
	studyGroupId int,
	...
	args := []interface{}{subjectId, studyGroupId, startAt, endAt}
	...
	query += `	
		...
		start_scores AS (
			...
			FROM all_sublessons sl
			LEFT JOIN level.level ll 
				ON ll.sub_lesson_id = sl.id
			LEFT JOIN level.level_play_log lpl 
				ON lpl.level_id = ll.id 
				AND lpl.played_at <= $3
		...
		SELECT DISTINCT ON ("a"."sub_lesson_name")
			a.sub_lesson_name AS scope,
			COALESCE(CASE
				WHEN COALESCE(s.total_stars, 0) = 0 AND COALESCE(e.total_stars, 0) != 0 THEN 100.00
			    WHEN COALESCE(s.total_stars, 0) = 0 THEN 0.00
				ELSE ((e.total_stars - s.total_stars) / s.total_stars::DECIMAL(10,2)) * 100
			END, 0) AS progress
		FROM all_sublessons a
		LEFT JOIN start_scores s 
			ON a.id = s.id
		LEFT JOIN end_scores e 
			ON a.id = e.id
	`
	...
	if pagination != nil {
		...
		query += fmt.Sprintf(` ORDER BY a.sub_lesson_name OFFSET $%d LIMIT $%d`, argsIndex, argsIndex+1)
