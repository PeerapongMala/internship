...
func (postgresRepository *postgresRepository) GetSubLessonProgressOverview(
	schoolId int,
	classIds []int,
	studyGroupIds []int,
	subjectIds []int,
	...
	query := `
		...
		start_scores AS (
			...
			LEFT JOIN level.level_play_log lpl 
				...
				AND lpl.played_at <= $4
				AND lpl.class_id = ANY($5)
			LEFT JOIN "class"."study_group" sg ON "lpl"."class_id" = "sg"."class_id"
			WHERE TRUE
		`
	args := []interface{}{schoolId, subjectIds, lessonIds, startAt, classIds, endAt, classIds}
