...
func (postgresRepository *postgresRepository) YearList(teacherId string, pagination *helper.Pagination) ([]constant.Year, error) {
	query := `
			SELECT DISTINCT ON ("sy"."short_name")
	...
	query += fmt.Sprintf(` ORDER BY "sy"."short_name" LIMIT $2 OFFSET $3`)
	response := []constant.Year{}
