...
func (postgresRepository *postgresTeacherStudentRepository) LevelOptionsGetByParams(in constant.OptionParam) ([]constant.OptionItem, error) {
	stm := `
	SELECT
		lvl.id,
		lvl.index AS label,
		lvl.sub_lesson_id AS parent_id
...
`
	...
	closingQuery := "ORDER BY lvl.index ASC"
	queryBuilder := helper.NewQueryBuilder(stm, in.Student.UserId, in.TeacherId)

	if in.AcademicYear != nil {
		queryBuilder.AddFilter("AND c.academic_yea =", in.AcademicYear)
	}
