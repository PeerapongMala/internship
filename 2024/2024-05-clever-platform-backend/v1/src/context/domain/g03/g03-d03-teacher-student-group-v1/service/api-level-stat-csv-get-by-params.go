...
func (api *APIStruct) LevelStatCsvGetByParams(context *fiber.Ctx) error {
	var (
	...
	data, err := api.Service.LevelStatCsvGetByParams(in)
	if err != nil {
	...
	context.Attachment("level_stat_study_group.csv")

	return context.SendStream(reader)
}

func (service *serviceStruct) LevelStatCsvGetByParams(in constant.LevelStatFilter) ([]byte, error) {
	data, err := service.LevelStatListGetByParams(in)
	if err != nil {
		return nil, err
	}

	csvMapped := [][]string{constant.GroupLevelStatCsvHeader}
	for _, item := range data {
		csvMapped = append(csvMapped, []string{
			...
			fmt.Sprintf("%d/%d", item.UserPlayCount.Value, item.UserPlayCount.Total),
