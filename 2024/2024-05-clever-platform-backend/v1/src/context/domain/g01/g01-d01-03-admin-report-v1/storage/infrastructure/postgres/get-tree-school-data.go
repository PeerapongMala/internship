...
func (postgresRepository *postgresRepository) GetTreeSchool(province, district, startDate, endDate string) ([]constant.TreeDistrictData, error) {
	query := `
		SELECT
			student_stat.id,
			student_stat.name,
	...
	err := postgresRepository.Database.Select(&entities, query, startDate, endDate, province, district)
	if err != nil {
	...
	for _, entity := range entities {
		resp = append(resp, constant.TreeDistrictData{
			SchoolId:     helper.Deref(entity.Id),
