...
func (postgresRepository *postgresRepository) DistrictProgressList(pagination *helper.Pagination, startDate, endDate *time.Time, parentScope string) ([]constant.ProgressReport, error) {
	query := `
		WITH "start_progress_reports" AS (
            ...
            FROM (
                ...
                WHERE
                	"sad"."district_zone" = $2
                GROUP BY "lpl"."student_id", "lpl"."level_id", "sad"."district"
            ) AS "sub"
            GROUP BY "district"
        ),
        ...
        WHERE "spr"."district" IS NOT NULL
	`
