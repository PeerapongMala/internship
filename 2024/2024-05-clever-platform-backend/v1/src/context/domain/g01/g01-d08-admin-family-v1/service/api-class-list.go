...
func (api *APIStruct) ClassList(context *fiber.Ctx) error {
	...
	data, err := api.Service.ClassList(filter, pagination)
	if err != nil {
		errMsg := err.Error()
...
func (service *serviceStruct) ClassList(filter *constant.DropdownFilter, pagination *helper.Pagination) ([]*constant.ClassList, error) {
	classList, err := service.adminFamilyStorage.ClassList(filter, pagination)
	if err != nil {
		return nil, err
	}
