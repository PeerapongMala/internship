...
func (api *APIStruct) SchoolList(context *fiber.Ctx) error {
	...
	data, err := api.Service.SchoolList(pagination)
	if err != nil {
		errMsg := err.Error()
...
func (service *serviceStruct) SchoolList(pagination *helper.Pagination) ([]*constant.SchoolList, error) {
	school, err := service.adminFamilyStorage.SchoolList(pagination)
	if err != nil {
		return nil, err
	}
	return school, nil
