...
type AnnouncerCaseUploadCsvRequest struct {
...
type AnnouncerCaseUploadCsvResponse struct {
...
func (api *APIStruct) AnnouncerCaseUploadCsv(context *fiber.Ctx) error {
	request, err := helper.ParseAndValidateRequest(context, &AnnouncerCaseUploadCsvRequest{}, helper.ParseOptions{Params: true, Body: true})
	if err != nil {
	...
	err = api.Service.AnnouncerCaseUploadCsv(&AnnouncerCaseUploadCsvInput{
		SubjectId:                     subjectId,
		SchoolId:                      request.SchoolId,
		AnnouncerCaseUploadCsvRequest: request,
	})
	if err != nil {
		return helper.RespondHttpError(context, err)
	}

	return context.Status(http.StatusOK).JSON(AnnouncerCaseUploadCsvResponse{
...
type AnnouncerCaseUploadCsvInput struct {
	SubjectId string
	SchoolId  int
	*AnnouncerCaseUploadCsvRequest
}

func (service *serviceStruct) AnnouncerCaseUploadCsv(in *AnnouncerCaseUploadCsvInput) error {
	...
	for {
		...
		if len(row) != len(constant.AnnouncerCsvHeader) {
		...
		if userEntity.Id == "" {
			...
			schoolAnnouncerEntity := constant.SchoolAnnouncerEntity{
				SchoolId: in.SchoolId,
				UserId:   savedUser.Id,
			}
			err = service.adminSchoolStorage.AnnouncerCaseAddSchool(tx, &schoolAnnouncerEntity)
			if err != nil {
				return err
			}

			_, err = service.adminSchoolStorage.UserCaseAddUserRole(tx, userEntity.Id, []int{int(constant.Announcer)})
			if err != nil {
				return err
			}
