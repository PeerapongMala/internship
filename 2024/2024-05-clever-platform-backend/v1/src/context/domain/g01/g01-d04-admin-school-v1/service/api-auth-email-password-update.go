...
func (service *serviceStruct) AuthEmailPasswordUpdate(in *AuthEmailPasswordUpdateInput) error {
	authEmailPassword, err := service.adminSchoolStorage.AuthEmailPasswordGet(in.UserId)
	...
	newAuthEmailPassword := constant.AuthEmailPasswordEntity{
		UserId:       in.UserId,
		PasswordHash: *passwordHash,
	}
	if authEmailPassword != nil {
		err = service.adminSchoolStorage.AuthEmailPasswordUpdate(nil, &newAuthEmailPassword)
		if err != nil {
			return err
		}
	} else {
		_, err = service.adminSchoolStorage.AuthEmailPasswordCreate(nil, &newAuthEmailPassword)
		if err != nil {
			return err
		}
	}
