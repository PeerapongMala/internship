...
func (api *APIStruct) ObserverUploadCSV(context *fiber.Ctx) error {
	...
	err = api.Service.ObserverUploadCSV(&request)
	if err != nil {
...
func (service *serviceStruct) ObserverUploadCSV(in *constant.UploadCSVInput) error {
	// observerAccessList, err := service.adminUserAccountStorage.ObserverAccessList(in.Pagination)
	// if err != nil {
	// 	return err
	// }
	...
	callback := func(record []string) error {
		...
		var observerAccesses []int
		if observerAccesses != nil {
			accesses := strings.Split(observerAccess, ",")
			for _, access := range accesses {
				id, err := strconv.Atoi(strings.TrimSpace(access))
				if err != nil {
					return err
				}
				observerAccesses = append(observerAccesses, id)
			}
		}

		// In case create observer
		if len(id) == 0 {
			...
			err = service.adminUserAccountStorage.ObserverCaseAddObserverAccess(tx, user.Id, observerAccesses)
			if err != nil {
				return err
			}

			// In case update observer
		} else {
			now := time.Now().UTC()
			userEntity.Id = id
			...
			err = service.adminUserAccountStorage.ObserverCaseUpdateObserverAccesses(tx, userEntity.Id, observerAccesses)
			if err != nil {
				return err
			}
	...
	err = helper.ReadCSVFileWithValidateHeaders(&file, callback, nil, constant.ObserverCSVHeader)
	if err != nil {
		return err
	}
