...
func (api *APIStruct) AdminUploadCSV(context *fiber.Ctx) error {
	...
	err = api.Service.AdminUploadCSV(&request)
	if err != nil {
...
func (service *serviceStruct) AdminUploadCSV(req *constant.UploadCSVInput) error {
	...
	callback := func(record []string) error {
		...
		roles_id := record[8]  // ความรับผิดชอบ
		...
		var roles_ID []int
		roles := strings.Split(roles_id, ",")
		for _, role := range roles {
			id, err := strconv.Atoi(strings.TrimSpace(role))
			if err != nil {
				return err
			}
			roles_ID = append(roles_ID, id)
		}
		...
		if len(id) == 0 {
			...
			_, err = service.adminUserAccountStorage.UserCaseAddUserRole(tx, userEntity.Id, roles_ID)
			if err != nil {
				return err
			}
			...
			err = service.adminUserAccountStorage.UserCaseUpdateUserRole(tx, userEntity.Id, roles_ID)
			if err != nil {
				return err
			}
	...
	err = helper.ReadCSVFileWithValidateHeaders(&file, callback, nil, constant.AdminCSVHeader)
	if err != nil {
		return err
	}
