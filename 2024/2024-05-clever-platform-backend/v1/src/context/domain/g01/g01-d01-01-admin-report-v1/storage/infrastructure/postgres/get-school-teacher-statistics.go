...
func (postgresRepository *postgresRepository) AdminProgressGetSchoolTeacherStatistics(
	schoolID int,
	...
	WITH "start_progress_reports" AS (
		...
		FROM (
			...
			GROUP BY "lpl"."student_id", "lpl"."level_id", "st"."user_id"
		) as "sub"
		GROUP BY "teacher_id"
	),
		...
		FROM (
			...
			FROM
				"school"."school_teacher" AS "st"
					...
					LEFT JOIN "level"."level_play_log" AS "lpl" ON "lpl"."class_id" = "ct"."class_id"	
						AND "lpl"."played_at" <= $3
			WHERE st.school_id = $4
			GROUP BY "lpl"."student_id", "lpl"."level_id", "st"."user_id"
		) as "sub"
		GROUP BY "teacher_id"
	)
	SELECT
		"spr"."teacher_id" AS "scope",
		CASE
	...
	LEFT JOIN "end_progress_reports" AS "epr"
		ON "spr"."teacher_id" = "epr"."teacher_id"
	`

	args := []interface{}{startDate, schoolID, endDate, schoolID}
