...
func (postgresRepository *postgresRepository) AdminProgressGetSchoolTeacherClasses(
	schoolID int,
	academicYear string,
	year string,
			...
			FROM
				"school"."school_teacher" AS "st"
					...
					LEFT JOIN "level"."level_play_log" AS "lpl" ON "lpl"."class_id" = "ct"."class_id"
						AND "lpl"."played_at" <= $1
			WHERE st.school_id = $2 and c.academic_year = $3 and c.year = $4
		...
		FROM (
			...
			FROM
				"school"."school_teacher" AS "st"
					...
					LEFT JOIN "level"."level_play_log" AS "lpl" ON "lpl"."class_id" = "ct"."class_id"
						AND "lpl"."played_at" <= $5
			WHERE st.school_id = $6 and c.academic_year = $7 and c.year = $8
			GROUP BY "lpl"."student_id", "lpl"."level_id", "class_name"
		) as "sub"
		GROUP BY "class_name"
	)
	...
	`
	args := []interface{}{startDate, schoolID, academicYear, year, endDate, schoolID, academicYear, year}
