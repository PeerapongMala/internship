...
import (
	...
	"github.com/ZettaMerge/2024-05-clever-platform-backend/src/context/domain/g04/g04-d05-redeem-v1/storage"
...
func Init(app *fiber.App, resource coreInterface.Resource, path string) service.ServiceInterface {
	...
	redeemStorageInstance := storage.StorageNew(resource)
	cloudStorageInstance := cloudStorage.StorageNew(resource)
	redeemServiceInstance := service.ServiceNew(redeemStorageInstance, cloudStorageInstance)

	route(app, authMiddleware, redeemServiceInstance, path)
