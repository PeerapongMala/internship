...
func (api *APIStruct) GetEventAnnounceById(context *fiber.Ctx) error {
	...
	response, err := api.Service.GetEventAnnounceById(Id, RolesCheck)
	if err != nil {
...
func (service *serviceStruct) GetEventAnnounceById(AnnounceId int, Role constant.CheckRoleRequest) (*constant.EventAnnounceResponse, error) {
	check := false
	...
	response, err := service.GmannounceStorage.GetEventAnnounceById(AnnounceId)
	if err != nil {
		if err.Error() == "not found" {
