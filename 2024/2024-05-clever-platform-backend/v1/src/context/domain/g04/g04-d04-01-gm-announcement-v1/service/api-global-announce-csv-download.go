...
func (api *APIStruct) GlobalAnnounceCsvDownload(context *fiber.Ctx) error {
	...
	fileBytes, err := api.Service.GlobalAnnounceCsvDownload(request, filter, check, pagination)
	if err != nil {
...
func (service *serviceStruct) GlobalAnnounceCsvDownload(req constant.CsvDowloadRequest, filter constant.SystemAnnounceFilter, Role constant.CheckRoleRequest, pagination *helper.Pagination) ([]byte, error) {
	check := false
	...
	response, _, err := service.GmannounceStorage.GetGlobalAnnounce(pagination, filter)
	if err != nil {
		return nil, err
	}
	...
	Data := [][]string{constant.GlobalAnnouncCsvHeader}
