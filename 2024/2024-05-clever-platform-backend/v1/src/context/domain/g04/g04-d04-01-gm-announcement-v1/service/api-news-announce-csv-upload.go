...
func (api *APIStruct) NewsAnnounceCSVUpload(context *fiber.Ctx) error {
	...
	err = api.Service.NewsAnnounceCsvUpload(request, Check)
	if err != nil {
...
func (service *serviceStruct) NewsAnnounceCsvUpload(req constant.CSVUploadRequest, Role constant.CheckRoleRequest) error {
	...
	callback := func(record []string) error {
		...
		if id != 0 {
			service.GmannounceStorage.UpdateNewsAnnounce(constant.UpdateNewsAnnounceRequest{
				Id:           id,
				SchoolId:     SchoolId,
				Scope:        row3,
				Type:         row4,
			...
			service.GmannounceStorage.AddNewsAnnounce(constant.CreateNewsAnnounceRequest{
				SchoolId:     SchoolId,
				Scope:        row3,
				Type:         row4,
				Title:        row5,
	...
	err = helper.ReadCSVFileWithValidateHeaders(&file, callback, nil, constant.NewsAnnounceCsvHeader)
	if err != nil {
		return err
	}
