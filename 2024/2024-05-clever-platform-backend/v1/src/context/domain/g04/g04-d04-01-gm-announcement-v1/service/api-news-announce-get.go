...
func (api *APIStruct) GetNewsAnnounceById(context *fiber.Ctx) error {
	...
	response, err := api.Service.GetNewsAnnounceById(Id, RolesCheck)
	if err != nil {
...
func (service *serviceStruct) GetNewsAnnounceById(AnnounceId int, Role constant.CheckRoleRequest) (*constant.NewsAnnounceResponse, error) {
	check := false
	...
	response, err := service.GmannounceStorage.GetNewsAnnounceById(AnnounceId)
	if err != nil {
		if err.Error() == "not found" {
