...
func (postgresRepository *postgresRepository) GetRewardAnnounce(pagination *helper.Pagination, filter constant.RewardAnnounceFilter) ([]constant.RewardAnnounceResponseService, int, error) {
	query := `
	SELECT
	"an"."id",
	"sch"."id" AS "school_id",
	...
	WHERE "an"."scope" = 'Subject' AND "an"."type" = 'reward'
	...
	if filter.AcademicYear != 0 {
		query += fmt.Sprintf(` AND "ar"."academic_year" = $%d `, argsIndex)
	...
	responses := []constant.RewardAnnounceResponseService{}
	for rows.Next() {
	...
	if filter.AcademicYear != 0 {
		countQuery += fmt.Sprintf(` AND "ar"."academic_year" = $%d `, countIndex)
