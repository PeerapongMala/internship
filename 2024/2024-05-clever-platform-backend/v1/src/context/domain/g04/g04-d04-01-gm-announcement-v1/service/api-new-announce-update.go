...
func (api *APIStruct) UpdateNewsAnnounce(context *fiber.Ctx) error {
	var body constant.UpdateNewsAnnounceRequest
	...
	body.Id = Id
	err = api.Service.UpdateNewsAnnounce(body, RolesCheck)
	if err != nil {
...
func (service *serviceStruct) UpdateNewsAnnounce(c constant.UpdateNewsAnnounceRequest, Role constant.CheckRoleRequest) error {
	response, err := service.GmannounceStorage.GetNewsAnnounceById(c.Id)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return err
	}
	...
	err = service.GmannounceStorage.UpdateNewsAnnounce(c)
	if err != nil {
		if err.Error() == "Announce id is not Exist" {
