...
func (api *APIStruct) RewardAnnounceCsvDownload(context *fiber.Ctx) error {
	...
	fileBytes, err := api.Service.RewardAnnounceCsvDownload(request, filter, check, pagination)
	if err != nil {
...
func (service *serviceStruct) RewardAnnounceCsvDownload(req constant.CsvDowloadRequest, filter constant.RewardAnnounceFilter, Role constant.CheckRoleRequest, pagination *helper.Pagination) ([]byte, error) {
	check := false
	...
	response, _, err := service.GmannounceStorage.GetRewardAnnounce(pagination, filter)
	if err != nil {
		return nil, err
	}
	...
	Data := [][]string{constant.RewardAnnounceCsvHeader}
