...
func (api *APIStruct) UpdateEventAnnounce(context *fiber.Ctx) error {
	var body constant.UpdateEventAnnounceRequest
	...
	body.Id = announceId
	err = api.Service.UpdateEventAnnounce(body, RolesCheck)
	if err != nil {
...
func (service *serviceStruct) UpdateEventAnnounce(c constant.UpdateEventAnnounceRequest, Role constant.CheckRoleRequest) error {
	response, err := service.GmannounceStorage.GetEventAnnounceById(c.Id)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return err
	}
	...
	err = service.GmannounceStorage.UpdateEventAnnounce(c)
	if err != nil {
		if err.Error() == "Announce id is not Exist" {
