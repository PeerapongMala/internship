...
func (api *APIStruct) NewsAnnounceCsvDownload(context *fiber.Ctx) error {
	...
	filter := constant.NewsAnnounceFilter{}
	err = context.QueryParser(&filter)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}
	fileBytes, err := api.Service.NewsAnnounceCsvDownload(request, filter, check, pagination)
	if err != nil {
...
func (service *serviceStruct) NewsAnnounceCsvDownload(req constant.CsvDowloadRequest, filter constant.NewsAnnounceFilter, Role constant.CheckRoleRequest, pagination *helper.Pagination) ([]byte, error) {
	check := false
	...
	response, _, err := service.GmannounceStorage.GetNewsAnnounce(pagination, filter)
	if err != nil {
		return nil, err
	}
	...
	Data := [][]string{constant.NewsAnnounceCsvHeader}
