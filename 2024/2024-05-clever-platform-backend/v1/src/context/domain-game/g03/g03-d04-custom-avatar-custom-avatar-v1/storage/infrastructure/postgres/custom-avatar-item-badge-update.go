...
func (postgresRepository *postgresRepository) UpdateCustomAvatarItemBadgeEquipped(c constant.UpdateCustomAvatarItemEquippedRequest) (r constant.InventoryAvatarItemBadgeEntity, err error) {

	// reset another inventory_avatar is_equipped = false
	queryUpdate := `UPDATE inventory.inventory_item as inv_item
					SET is_equipped  = false  
					WHERE 
					(select item.type from item.item as item where item.id = inv_item.item_id) = 'badge'
	...
	queryUpdate = `UPDATE inventory.inventory_item as inv_item
					SET is_equipped = $1
					WHERE (select item.type from item.item as item where item.id = inv_item.item_id) = 'badge'
