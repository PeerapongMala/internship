...
func (postgresRepository *postgresRepository) UpdateCustomAvatarItemFrameEquipped(c constant.UpdateCustomAvatarItemEquippedRequest) (r constant.InventoryAvatarItemEntity, err error) {
	// reset another inventory_avatar is_equipped = false
	queryUpdate := `UPDATE inventory.inventory_item as inv_item
					SET is_equipped  = false  
					WHERE (select item.type from item.item as item where item.id = inv_item.item_id) = 'frame'
	...
	queryUpdate = `UPDATE inventory.inventory_item
					SET is_equipped = $1
					WHERE (select item.type from item.item as item where item.id = item_id) = 'frame'
					AND item_id = $2 
