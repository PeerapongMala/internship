...
func (postgresRepository *postgresRepository) GetCustomAvatarItemCoupon(c constant.GetInventoryAvatarItemRequest) (r []constant.InventoryAvatarItemEntity, err error) {

	query := `SELECT
		inv_item.*, item.image_url, item.status, item.name, item.description
	FROM
