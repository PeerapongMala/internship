...
func (api *APIStruct) UpdateShopItem(context *fiber.Ctx) error {
	...
	response, err := api.Service.UpdateShopItem(*request)
	if err != nil {
		return helper.RespondHttpError(context, err)
	}
...
func (service *serviceStruct) UpdateShopItem(c constant.ShopItemRequest) (r constant.ShopItemResponse, err error) {
	return service.shopStorage.UpdateShopItem(c)
}
