...
// ==================== Endpoint ==========================
func (api *APIStruct) GetFamilyInfo(context *fiber.Ctx) error {
	userId, ok := context.Locals("subjectId").(string)
	if !ok {
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}

	data, err := api.Service.GetFamilyInfo(userId)
	if err != nil {
		errMsg := err.Error()
...
// ==================== Service ==========================
func (service *serviceStruct) GetFamilyInfo(userID string) (*constant.FamilyInfo, error) {
	account, err := service.settingsStorage.GetFamilyInfo(userID)
	if err != nil {
		return nil, err
	}
	return account, nil
