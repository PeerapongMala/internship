...
func (api *APIStruct) EventReadAll(context *fiber.Ctx) error {
	...
	err = api.Service.EventReadAll(subjectId, SubjectId)
	if err != nil {
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}
...
func (service *serviceStruct) EventReadAll(userId string, subjectId int) error {
	...
	AnnouncementList, _, err := service.mailBoxStorage.EventAnnounceList(req, pagination)
	if err != nil {
		return err
	}
