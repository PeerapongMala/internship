...
func (api *APIStruct) SystemAnnounceList(context *fiber.Ctx) error {
	...
	response, totalCount, err := api.Service.SystemAnnounceList(req, pagination)
	if err != nil {
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}
...
func (service *serviceStruct) SystemAnnounceList(req constant.AnnouncementListRequest, pagination *helper.Pagination) ([]constant.SystemAnnouncement, int, error) {
	...
	response, totalCount, err := service.mailBoxStorage.SystemAnnounceList(request, pagination)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, 0, err
	}
