...
func (postgresRepository *postgresRepository) GetAchivementByUserAndSubjectId(userId string, subjectId int) ([]constant.SpecialRewardWithDataEntity, error) {
	query := `
		SELECT 
	...
	err := postgresRepository.Database.Select(&entities, query, subjectId, userId)
	if err != nil {
