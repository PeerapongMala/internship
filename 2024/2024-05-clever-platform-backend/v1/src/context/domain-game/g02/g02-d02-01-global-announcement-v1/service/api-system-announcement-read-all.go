...
func (api *APIStruct) SystemReadAll(context *fiber.Ctx) error {
	...
	err := api.Service.SystemReadAll(subjectId)
	if err != nil {
		return helper.RespondHttpError(context, helper.NewHttpError(http.StatusInternalServerError, nil))
	}
...
func (service *serviceStruct) SystemReadAll(userId string) error {
	...
	response, _, err := service.globalAnnounceStorage.SystemAnnouncementList(schoolId, pagination)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return err
	}
