...
func (api *APIStruct) TeacherAnnouncementList(context *fiber.Ctx) error {
	...
	response, totalCount, err := api.Service.TeacherAnnoucementList(subjectId, pagination)
	if err != nil {
...
func (service *serviceStruct) TeacherAnnoucementList(userId string, pagination *helper.Pagination) ([]constant.AnnouncementResponse, int, error) {
	schoolId, err := service.globalAnnounceStorage.GetSchoolIdByStudentId(userId)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, 0, err
	}
	response, totalCount, err := service.globalAnnounceStorage.TeacherAnnoucementList(schoolId, pagination)
	if err != nil {
		log.Printf("%+v", errors.WithStack(err))
		return nil, 0, err
	}
