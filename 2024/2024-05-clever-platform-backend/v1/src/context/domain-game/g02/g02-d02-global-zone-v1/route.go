...
func route(app *fiber.App, authMiddleware *middleware.AuthMiddleware, serviceInstance service.ServiceInterface, path string) {
	...
	route.Get("/select-subject", api.GlobalZoneSelectSubjectGet)
	route.Get("/announcement", api.AnnouncementGet)
	route.Post("/announcement/:announcementId", api.AnnouncementUpdateStatus)
}
