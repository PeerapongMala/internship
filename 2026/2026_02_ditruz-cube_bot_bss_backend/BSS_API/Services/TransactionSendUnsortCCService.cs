    ...
    public class TransactionSendUnsortCCService(IUnitOfWork unitOfWork) : ITransactionSendUnsortCCService
    {
            ...
            CancellationToken ct = default)
        {
            try
            {
                ...
                createSendUnsortResponse.RegisterUnsort = allRegisterUnsort.OrderByDescending(w => w.CreatedDate).ToList();
                    ...
                    if (transactionSendUnsortCc != null)
                    {
                        ...
                        foreach (var transactionSendUnsortCcItem in transactionSendUnsortCc)
                        {
                            RegisterSendUnsortCCResponse registerSendUnsortCCResponse = new RegisterSendUnsortCCResponse
                            {
                                ...
                                CanEdit = transactionSendUnsortCcItem.StatusId != BssStatusConstants.Delivered,
                                CanPrint = transactionSendUnsortCcItem.StatusId == BssStatusConstants.Delivered,
                                CanDelete = transactionSendUnsortCcItem.StatusId != BssStatusConstants.Delivered &&
                                            transactionSendUnsortCcItem.StatusId != BssStatusConstants.DeliveredNote,
            ...
            try
            {
                ...
                if (transactionSendUnsortCC.TransactionSendUnsortData is { Count: > 0 })
                {
                    foreach (var transactionSendUnsortData in transactionSendUnsortCC.TransactionSendUnsortData)
                    {
                        ...
                        var oldStatus = (BssStatusEnum)sendUnsortDataResponse.StatusId;
                        var oldStatusTarget = BssStatusEnum.CorrectReturn | // แก้ไขรายการส่งกลับ
                                              BssStatusEnum.Returned; // ส่งคืน/ไม่รับมอบ

                        sendUnsortDataResponse.CanEdit = (oldStatus & oldStatusTarget) != 0;
                ...
                if (editSendUnsortDeliveryResponse.StatusId == BssStatusConstants.DeliveredNote)
                {
                    ...
                    DateTime startDate = configList.ToScanPrepareBssWorkDayStartDateTime();
                    DateTime endDate = configList.ToScanPrepareBssWorkDayEndDateTime();
                    ...
                    if (transactionRegisterUnsort is { Count: > 0 })
                    {
                        foreach (var transactionRegisterUnsortItem in transactionRegisterUnsort)
                        {
                            ...
                            var newStatus = (BssStatusEnum)sendUnsortDataResponse.StatusId;
                            var newStatusTarget = BssStatusEnum.CorrectReturn | // แก้ไขรายการส่งกลับ
                                                  BssStatusEnum.Returned; // ส่งคืน/ไม่รับมอบ

                            sendUnsortDataResponse.CanEdit = (newStatus & newStatusTarget) != 0;
                            editSendUnsortDeliveryResponse.SendUnsortData.Add(sendUnsortDataResponse);
                        }
                    }
                }
            ...
            try
            {
                ...
                if (confirmEditSendUnsortDeliveryRequest.SendUnsortData is { Count: > 0 })
                {
                    ...
                    foreach (var oldRegisterUnsort in oldSelectedRegisterUnsort)
                    {
                        ...
                        if (oldExistingTransactionRegisterUnsort == null)
                        {
                            throw new Exception("register unsort not found");
                        }
                        ...
                        if (oldRegisterUnsort.IsSelected &&
                            oldExistingTransactionRegisterUnsort.ContainerCode.TrimEnd() !=
                        ...
                        if (oldRegisterUnsort is { IsSelected: true, UnsortCC.Count: > 0 })
                        {
                        ...
                        {   
                            ...
                            oldExistingTransactionRegisterUnsort.UpdatedBy = confirmEditSendUnsortDeliveryRequest.UserId;
                            oldExistingTransactionRegisterUnsort.UpdatedDate = DateTime.Now;
                        }
                    ...
                    if (confirmEditSendUnsortDeliveryRequest.StatusId == BssStatusConstants.DeliveredNote)
                    {
                        ...
                        foreach (var newRegisterUnsort in newSelectedRegisterUnsort)
                        {
                            ...
                            if (newRegisterUnsort.IsSelected &&
                                newExistingTransactionRegisterUnsort.ContainerCode.TrimEnd() !=
                                newRegisterUnsort.ContainerCode.TrimEnd())
                            {
                                ...
                                newExistingTransactionRegisterUnsort.ContainerCode = newRegisterUnsort.ContainerCode;
                            }

                            if (newRegisterUnsort is { IsSelected: true, UnsortCC.Count: > 0 })
                            {
