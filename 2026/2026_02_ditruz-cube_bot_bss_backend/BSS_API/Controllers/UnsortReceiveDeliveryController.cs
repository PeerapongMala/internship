namespace BSS_API.Controllers
{
        ...
        : BaseController(share)
    {
        ...
        public async Task<IActionResult> GetReceiveBySendUnsortCode([FromQuery] string SendUnsortCode, [FromQuery] int departmentId)
        {
            ...
            if (data == null)
            {
                return ApiDataNotFound("ไม่พบข้อมูลใบนำส่งที่สแกน");
            }
        ...
        public async Task<IActionResult> ExecuteReceive([FromBody] DeliveryActionRequest request)
        {
            try
            {
                var result = await _UnsortReceiveDeliveryService.ExecuteReceiveDelivery(request.SendUnsortId, request.UserId);
        ...
        public async Task<IActionResult> ExecuteReject([FromBody] DeliveryActionRequest request)
        {
            try
            {
                ...
                var result = await _UnsortReceiveDeliveryService.ExecuteRejectDelivery(request.SendUnsortId, request.UserId, request.Note);
        ...
        public async Task<IActionResult> ExecuteReturn([FromBody] BulkDeliveryActionRequest request)
        {
            try
            {
                ...
                var result = await _UnsortReceiveDeliveryService.ExecuteReturnDelivery(request.SendUnsortIds, request.UserId, request.Note);
                ...
                return ApiSuccess("บันทึกการส่งคืนเรียบร้อยแล้ว");
            }
            catch (Exception ex)
            {
                return ApiInternalServerError(ex.Message);
