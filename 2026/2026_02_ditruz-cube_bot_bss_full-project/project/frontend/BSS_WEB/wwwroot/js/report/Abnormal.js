...
async function loadHeaderCardList(machineId) {
    const selectElem = $('#headercardSelect');
    const dateSelect = getSelectedDateForApi(); // ดึงวันที่ ค.ศ.

    // 1. ระหว่างรอโหลด ให้ Disable และแสดงสถานะ
    selectElem.prop("disabled", true).html('<option value="">กำลังค้นหาข้อมูล...</option>').trigger('change');
    ...
    try {
        ...
        const response = await fetch(`/Report/GetHeaderCardList?${params.toString()}`);
        const result = await response.json();

        let options = '';

        if (result.is_success && Array.isArray(result.data) && result.data.length > 0) {
            // กรณีมีข้อมูล: บังคับให้เลือกตัวแรก หรือใส่ placeholder
            options = '<option value="">-- กรุณาเลือก Header Card --</option>';
            result.data.forEach(item => {
                options += `<option value="${item.value}">${item.text}</option>`;
            });
        } else {
            // กรณีไม่มีข้อมูลที่ตรงตามเงื่อนไข
            options = '<option value="">-- ไม่พบข้อมูลในวันที่/เครื่องที่เลือก --</option>';
        }

        selectElem.html(options).prop("disabled", false).trigger('change');

    } catch (error) {
        console.error('Error fetching header cards:', error);
        selectElem.html('<option value="">เกิดข้อผิดพลาดในการโหลด</option>').prop("disabled", false).trigger('change');
    }
}
