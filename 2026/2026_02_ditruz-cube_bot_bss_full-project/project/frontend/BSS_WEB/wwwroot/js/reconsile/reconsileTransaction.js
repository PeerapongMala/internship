...
function clearAfterReconciliation() {
    ...
    // Clear hidden field so refresh won't reload data
    var tranIdField = document.getElementById('reconsileTranId');
    if (tranIdField) tranIdField.value = '0';
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (USE_MOCK_DATA) {
        ...
        // Real mode: page receives reconsileTranId via path param from p01
        ...
        if (sessionStorage.getItem('reconciled')) {
            ...
            var tranIdEl = document.getElementById('reconsileTranId');
            var tranId = tranIdEl ? tranIdEl.value : null;
            if (tranId && tranId !== '0') {
...
function handleChildHcScan() {
    ...
    $.ajax({ url: checkUrl, type: 'GET', dataType: 'json' }).done(function (res) {
        if (res && res.is_success && res.data) {
            ...
            if (res.data.hasMachineData) {
                input.value = '';
                showSwappedWarningAndRedirect(headerCardCode);
                return;
            }
            // UC06: เช็คว่า child HC ตรงกับ parent HC (ชนิดราคา, ธนาคาร, cashpoint)
            var parentRow = denomTableData[0];
            if (parentRow) {
                var mismatches = [];
                if (res.data.denomPrice && res.data.denomPrice !== parentRow.denom) {
                    mismatches.push('ชนิดราคา');
                }
                if (res.data.bankName && res.data.bankName !== parentRow.bank) {
                    mismatches.push('ธนาคาร');
                }
                if (res.data.cashpointName && res.data.cashpointName !== parentRow.cashpoint) {
                    mismatches.push('Cash Point');
                }
                if (mismatches.length > 0) {
                    input.value = '';
                    showErrorModal('Header Card นี้ไม่ตรงกับ Parent HC: ' + mismatches.join(', '));
                    return;
                }
            }
        }
...
function saveToSummary() {
    ...
    var maxQty = totalBundles > 1 ? totalBundles * 1000 + 99 : 1000;
    if (qty > maxQty) {
        showErrorModal('มีธนบัตรมากสุดได้แค่ ' + maxQty.toLocaleString() + ' ใบ (' + totalBundles + ' มัด)');
        inputQty.focus();
        return;
    }
...
function showEditConfirmation() {
    ...
    var maxQty = totalBundles > 1 ? totalBundles * 1000 + 99 : 1000;
    if (newQty > maxQty) {
        showInlineError('editFormError', 'มีธนบัตรมากสุดได้แค่ ' + maxQty.toLocaleString() + ' ใบ (' + totalBundles + ' มัด)');
        return;
    }
...
function performReconsile() {
    ...
    // เช็คจำนวนฉบับรวมตามสูตรจำนวนมัด
    var totalQty = summaryTableData.reduce(function (sum, r) { return sum + r.qty; }, 0);
    if (totalBundles > 1) {
        var minQty = (totalBundles - 1) * 1000 + 100;
        var maxQty = totalBundles * 1000 + 99;
        if (totalQty < minQty || totalQty > maxQty) {
            showErrorModal('จำนวนธนบัตรรวม ' + totalQty.toLocaleString() + ' ใบ ต้องอยู่ระหว่าง ' + minQty.toLocaleString() + ' - ' + maxQty.toLocaleString() + ' ใบ (' + totalBundles + ' มัด)');
            return;
        }
    } else {
        if (totalQty > 1000) {
            showErrorModal('จำนวนธนบัตรรวม ' + totalQty.toLocaleString() + ' ใบ เกิน 1,000 ใบ สำหรับ 1 มัด');
            return;
        }
    }
