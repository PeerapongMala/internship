...
window.addEventListener('load', async () => {
    ...
    try {
        ...
        if (container) {
            ...
            await window.exportPdfByPlaywright(content, 'PreparationUnsortCC_Report', true);
        }
    } catch (error) {
...
async function exportData() {
    ...
    if (selected.value === 'pdf') {
        //await exportPdf('report-body', 'preparation-unsort-cc-report');
        const container = document.getElementById('report-body');
        if (container) {
            const content = container.innerHTML;
            // เรียกใช้ฟังก์ชันจาก window
            await window.exportPdfByPlaywright(content, 'PreparationUnsortCC_Report');
        }
    }
...
//    window.location.href = `/Report/PreparationUnsortCCExportToExcel${window.location.search}`;
//}
...
async function exportExcelFromHtml() {
    ...
    try {
        ...
        form.method = 'POST';
        form.action = '/Report/PreparationUnsortCCExportToExcel';
