...
// Second screen
let pageTwoWindowAutoSelling = null;
...
function openSecondScreen() {
    ...
    var modal = new bootstrap.Modal(document.getElementById('secondScreenModal'));
    modal.show();

    // Init Select2 after modal is shown (needs visible DOM)
    $('#secondScreenModal').one('shown.bs.modal', function () {
        $('#dropdownDisplayTwo').select2({
            theme: 'bootstrap-5',
            minimumResultsForSearch: Infinity, // ปิด search
            dropdownParent: $('#secondScreenModal'),
            placeholder: '-- กรุณาเลือก --',
            allowClear: false
        }).on('change', function () {
            updateSecondScreenPreview();
        });
    });
}

function setupDropdownForSecondScreen() {
    var select = document.getElementById('dropdownDisplayTwo');
    if (!select) return;

    // Destroy existing Select2 instance before re-populating
    if ($.fn.select2 && $('#dropdownDisplayTwo').data('select2')) {
        $('#dropdownDisplayTwo').select2('destroy');
    }
    ...
    allData.forEach(function (item) {
        var cashpoint = (item.Cashpoint || '').trim();
        var bank = (item.Bank || '').trim();
        var hc = (item.HeaderCardCode || '').trim();
        var deno = item.DenominationPrice || 0;

        var val = cashpoint + '|' + bank + '|' + hc + '|' + deno;
        var label = 'ภาชนะ: ' + hc + ', ธนาคาร: ' + bank + ', ราคา: ' + deno;

        if (!items.find(function (i) { return i.val === val; })) {
            items.push({ val: val, label: label });
        }
    });
...
function getDenoColors(deno) {
    var d = parseInt(deno) || 0;
    switch (d) {
        case 1000: return { bg: '#FBF8F4', color: '#4F3E2B', border: '#9f7d57', outerBorder: '#c8b99a', outerBg: '#f5f0e8' };
        case 500:  return { bg: '#F8F5FF', color: '#3D2E5B', border: '#7b6b9b', outerBorder: '#b8aed0', outerBg: '#eee8f8' };
        case 100:  return { bg: '#FFE8E8', color: '#732E2E', border: '#a06060', outerBorder: '#d4a8a8', outerBg: '#f8e0e0' };
        case 50:   return { bg: '#F0F8FF', color: '#013665', border: '#5a8aaa', outerBorder: '#a8c8d8', outerBg: '#e0eef8' };
        case 20:   return { bg: '#F1F9F1', color: '#336C32', border: '#6a9a6a', outerBorder: '#a8c8a8', outerBg: '#e0f0e0' };
        default:   return { bg: '#f8f9fa', color: '#333', border: '#ccc', outerBorder: '#c8d0d8', outerBg: '#fff' };
    }
}

function updateSecondScreenPreview() {
    var select = document.getElementById('dropdownDisplayTwo');
    var preview = document.getElementById('secondScreenPreview');
    var val = select ? select.value : '';

    if (!val) {
        // Show empty border only
        document.getElementById('previewCashpoint').textContent = '\u00A0';
        document.getElementById('previewBank').textContent = '\u00A0';
        document.getElementById('previewDeno').textContent = '\u00A0';
        document.getElementById('previewHeaderCard').textContent = '\u00A0';
        var defCard = document.getElementById('previewCard');
        defCard.style.backgroundImage = 'none';
        defCard.style.background = '#fff';
        defCard.style.borderColor = '#ccc';
        return;
    }

    var parts = val.split('|');
    var cashpoint = parts[0] || '-';
    var bank = parts[1] || '-';
    var hc = parts[2] || '-';
    var deno = parts[3] || '-';

    var colors = getDenoColors(deno);

    document.getElementById('previewCashpoint').textContent = cashpoint;
    document.getElementById('previewBank').textContent = bank;
    document.getElementById('previewDeno').textContent = '฿' + deno;
    document.getElementById('previewHeaderCard').textContent = hc;

    var card = document.getElementById('previewCard');
    card.style.backgroundImage = 'linear-gradient(' + colors.bg + 'cc, ' + colors.bg + 'cc), var(--pattern-bg)';
    card.style.backgroundSize = 'cover';
    card.style.backgroundPosition = 'center';
    card.style.borderColor = colors.border;

    document.getElementById('previewCashpoint').style.color = colors.color;
    document.getElementById('previewBank').style.color = colors.color;
    document.getElementById('previewDeno').style.color = colors.color;
    document.getElementById('previewHeaderCard').style.color = colors.color;

    preview.style.display = 'block';
}

function confirmSecondScreen() {
    var select = document.getElementById('dropdownDisplayTwo');
    var selectedValue = select ? select.value : '';

    if (!selectedValue) {
        alert('กรุณาเลือกข้อมูลที่ต้องการแสดง');
        return;
    }

    // Parse selected value: cashpoint|bank|headerCard|deno
    ...
    // Close modal
    bootstrap.Modal.getInstance(document.getElementById('secondScreenModal'))?.hide();

    // Window settings
    var screenWidth = 1440;
    var screenHeight = 900;
    var leftPos = (screen.width - screenWidth) / 2;
    var topPos = (screen.height - screenHeight) / 2;
    var rootPath = document.body.getAttribute('data-root-path') || '/';
    var pageUrl = rootPath + 'AutoSelling/SecondScreenAutoSelling';
    var pageName = 'DisplayTwoAutoSellingWindow';
    var screenSettings = 'width=' + screenWidth + ',height=' + screenHeight +
    ...
    if (pageTwoWindowAutoSelling && !pageTwoWindowAutoSelling.closed) {
        pageTwoWindowAutoSelling.focus();
        updateSecondScreenContent();
        return;
    }

    // Open new popup
    pageTwoWindowAutoSelling = window.open(pageUrl, pageName, screenSettings);
    if (!pageTwoWindowAutoSelling) return;

    pageTwoWindowAutoSelling.onload = function () {
        setTimeout(function () {
            updateSecondScreenContent();
        }, 200);
    };
...
function updateSecondScreenContent() {
    if (!pageTwoWindowAutoSelling || pageTwoWindowAutoSelling.closed) return;

    var doc = pageTwoWindowAutoSelling.document;
    if (!doc || !doc.getElementById('displayCashpoint')) return;
    ...
    $(doc).find('#displayDeno').text(displayTwoDeno ? '฿' + displayTwoDeno : '-');
    $(doc).find('#displayContainerId').text(displayTwoContainerId || '-');

    // Trigger refresh (color + marquee)
