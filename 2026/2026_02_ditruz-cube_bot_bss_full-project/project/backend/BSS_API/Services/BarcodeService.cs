namespace BSS_API.Services
{
    ...
    public class BarcodeService : IBarcodeService
    {
            ...
            try
            {
                    ...
                    if (lastPrepareFromDatabase != null)
                    {
                        ...
                        else
                        {
                            numberOfBarcodeWrapToGenerate =
                                StringHelper.GetLast4DigitFromStringAndConvertToInt(lastPrepareFromDatabase
                                    .PackageCode);
                            if (bnType == BNType.UnsortCC || bnType == BNType.UnsortCAMember)
                            {
                ...
                switch (bnType)
                {
                    ...
                    case BNType.UnsortCAMember:
                        ...
                        var runningNoCaMember = numberOfBarcodeWrapToGenerate.Value;

                        preview.PackageCode = GenerateBarcodeWrapForCaMember(masterInstitution,
                            masterCompanyDepartment,
                            masterCashType, masterDenomination, _transactionPreparation.PrepareDate,
                            runningNoCaMember);

                        var isDuplicateCaMember = await _unitOfWork.TransactionPreparationRepos
                            .ExistsPackageCodeAsync(preview.PackageCode);

                        if (isDuplicateCaMember && isFirstScan)
                        {
                            var prefix = preview.PackageCode.Substring(0, preview.PackageCode.Length - 4);
                            ...
                            runningNoCaMember = lastRunning + 1;
                        ...
                        preview.BundleCode = GenerateBarcodeBundle(preview.PackageCode, masterCashPoint,
                            numberOfBarcodeBundleToGenerate.Value, zone: masterZone);
                        preview.WrapSequence = runningNoCaMember;
                        preview.BundleSequence = numberOfBarcodeBundleToGenerate.Value;
            ...
            try
            {
                ...
                switch (bnType)
                {
                    ...
                    case BNType.UnsortCAMember:
                        ...
                        var isDuplicateUnsortCAmember = await _unitOfWork.TransactionPreparationRepos
                            .ExistsPackageCodeAsync(_transactionPreparation.PackageCode);

                        if (isDuplicateUnsortCAmember && isFirstScan)
                        {
                ...
                if (isValidateAfterGenerate)
                {
                    switch (bnType)
                    {
                        ...
                        case BNType.UnsortCAMember:
                            ...
                            var unsortCAMemberBarcodeBundleValidateResult =
                                await ValidateBarcodeBundleAsync(unsortCAMemberBarcodeBundleValidateRequest);
                        ...
                        case BNType.UnsortCC:
                            ...
                            var unsortCCBarcodeWrapValidateResult =
                                await ValidateBarcodeWrapAsync(unsortCCBarcodeWrapValidateRequest);
                            if (!unsortCCBarcodeWrapValidateResult.IsValid)
                            {
                                throw new Exception(unsortCCBarcodeWrapValidateResult.ErrorMessage);
                            }
        ...
        /// <returns></returns>
        private string GenerateBarcodeWrapForCaMember(MasterInstitution institution,
            MasterCompanyDepartment companyDepartment,
            MasterCashType cashType,
            MasterDenomination denomination, DateTime prepareDate, int sequence)
        {
            var prepareDateString = DateTimeHelper.ToBuddhistDateYYMMDD(prepareDate);
        ...
        /// <param name="sequence"></param>
        /// <returns></returns>
        private string GenerateBarcodeWrapForUnsortCC(MasterInstitution institution,
            MasterCompanyDepartment companyDepartment,
            MasterCashType cashType,
            MasterDenomination denomination, DateTime prepareDate, int sequence)
        {
            ...
            ValidateBarcodeRequest validateBarcodeRequest)
        {
            ...
            if (!string.IsNullOrEmpty(barcodeContainer.BarcodeValue) && barcodeContainer.BarcodeValue.Length == 7)
            {
                ...
                if (validateBarcodeRequest.ValidateExistingInDatabase)
                {
                                ...
                                if (masterCompanyInstitution == null)
                                {
                                    ...
                                    validateBarcodeResponse.ErrorMessage =
                                        string.Format(ValidateBarcodeErrorMessage.ValidateCompanyInstitutionInvalid,
                                            masterInstitution?.InstitutionCode ?? string.Empty);
                                        ...
                                        if (masterCashCenter == null)
                                        {
                                            ...
                                            validateBarcodeResponse.ErrorMessage =
                                                string.Format(ValidateBarcodeErrorMessage.ValidateCashCenterInvalid,
                                                    masterCashCenter?.CashCenterCode ?? string.Empty);
                                            return validateBarcodeResponse;
                                        }
                                    }
                                }
                    ...
                    {
                        if (!validateBarcodeRequest.DepartmentId.HasValue || validateBarcodeRequest.DepartmentId == 0)
                        {
                            throw new ArgumentNullException(nameof(validateBarcodeRequest.DepartmentId));
                        }

                        #region ValidateInRegisterUnsort

                        List<int> inStatus =
                        [
                            BssStatusConstants.Registered, // 1
                            BssStatusConstants.DeliveredNote, // 2
                            BssStatusConstants.Delivered, // 3
                            BssStatusConstants.CorrectReturn, // 4
                            BssStatusConstants.NotAccepted, // 5
                            BssStatusConstants.Returned // 31
                        ];

                        var isFoundContainerIsSystem = await _unitOfWork.TransactionRegisterUnsortRepos.GetAsync(w =>
                            w.ContainerCode.Trim() == barcodeContainer.BarcodeValue.Trim() &&
                            inStatus.Contains(w.StatusId) &&
                            w.DepartmentId == validateBarcodeRequest.DepartmentId.Value &&
                            w.IsActive == true, tracked: false) != null;

                        if (!isFoundContainerIsSystem)
                        {
                            inStatus = [BssStatusConstants.Received]; // 6
                            isFoundContainerIsSystem = await _unitOfWork.TransactionRegisterUnsortRepos.GetAsync(w =>
                                w.ContainerCode.Trim() == barcodeContainer.BarcodeValue.Trim() &&
                                inStatus.Contains(w.StatusId) &&
                                w.CreatedDate >= startDate && w.CreatedDate <= endDate &&
                                w.IsActive == true, tracked: false) != null;
                        }

                        #endregion ValidateInRegisterUnsort

                        #region ValidateInContainerPrepare

                        if (!isFoundContainerIsSystem)
                        {
                            List<int> bnType =
                            [
                                BNTypeConstants.Unfit, BNTypeConstants.UnsortCAMember, BNTypeConstants.UnsortCANonMember
                            ];
                            isFoundContainerIsSystem =
                                await _unitOfWork.TransactionContainerPrepareRepos.GetAsync(
                                    w => w.ContainerCode.Trim() == barcodeContainer.BarcodeValue.Trim() &&
                                         w.CreatedDate >= startDate && w.CreatedDate <= endDate &&
                                         bnType.Contains(w.BntypeId) &&
                                         w.DepartmentId == validateBarcodeRequest.DepartmentId.Value &&
                                         w.IsActive == true, tracked: false) != null;
                        }

                        #endregion ValidateInContainerPrepare
                ...
                if (validateBarcodeRequest is { MachineId: not null, DepartmentId: not null } &&
                    validateBarcodeRequest.BssBNTypeCode != BssBNTypeCodeConstants.RegisterUnsortCC)
                {
            ...
            ValidateBarcodeRequest validateBarcodeRequest)
        {
                ...
                headerCard.BarcodeValue.All(char.IsDigit))
            {
                ...
                // Todo ต้องมี parameter แค่อันใดอันนึง **เปลี่ยนเป็นเช็คแค่ DepartmentId**
                if (validateBarcodeRequest is { DepartmentId: null })
                {
                    validateBarcodeResponse.IsValid = false;
                ...
                #region ValidateRemainingQty

                if (!string.IsNullOrEmpty(validateBarcodeRequest.ContainerId) &&
                    (validateBarcodeRequest.ReceiveId is not null || validateBarcodeRequest.UnSortCcId is not null))
                {
                    if (validateBarcodeRequest.BssBNTypeCode == BssBNTypeCodeConstants.Unfit ||
                        validateBarcodeRequest.BssBNTypeCode == BssBNTypeCodeConstants.UnsortCANonMember ||
                        validateBarcodeRequest.BssBNTypeCode == BssBNTypeCodeConstants.UnsortCAMember)
                    {
                        var bnTypeCodeInput = validateBarcodeRequest.BssBNTypeCode == BssBNTypeCodeConstants.Unfit
                            ? BNTypeCodeConstants.Unfit
                            : BNTypeCodeConstants.UnsortCAMember;

                        // Check RemainingQty in Container
                        bool hasRemainingInContainer = await _unitOfWork.CbmsTransactionRepos
                            .AnyAsync(w =>
                                w.DepartmentId == validateBarcodeRequest.DepartmentId &&
                                w.ContainerId == validateBarcodeRequest.ContainerId &&
                                w.BnTypeInput == bnTypeCodeInput &&
                        ...
                        if (!hasRemainingInContainer)
                        {
                            validateBarcodeResponse.IsValid = false;
                            validateBarcodeResponse.IsAllRemainingZero = true;
                            validateBarcodeResponse.ErrorMessage = ValidateBarcodeErrorMessage.HeaderCardIsAllRemainingZero;
                            return validateBarcodeResponse;
                        }

                        // Check RemainingQty in Container By ReceiveId
                        bool hasRemainingByReceiveId = await _unitOfWork.CbmsTransactionRepos
                            .AnyAsync(w =>
                                w.DepartmentId == validateBarcodeRequest.DepartmentId &&
                                w.ContainerId == validateBarcodeRequest.ContainerId &&
                                w.ReceiveId == validateBarcodeRequest.ReceiveId &&
                                w.BnTypeInput == bnTypeCodeInput &&
                                w.RemainingQty > 0);

                        if (!hasRemainingByReceiveId)
                        {
                            validateBarcodeResponse.IsValid = false;
                            validateBarcodeResponse.IsRemainingZero = true;
                            validateBarcodeResponse.ErrorMessage = ValidateBarcodeErrorMessage.HeaderCardIsRemainingZero;
                            return validateBarcodeResponse;
                        }
                    }
                    else if (validateBarcodeRequest.BssBNTypeCode == BssBNTypeCodeConstants.UnsortCC)
                    {
                        var registerUnsort = await _unitOfWork.TransactionRegisterUnsortRepos.GetAsync(w =>
                                w.ContainerCode == validateBarcodeRequest.ContainerId && w.IsActive == true,
                            tracked: false);
                        ...
                        if (registerUnsort == null)
                        {
                            validateBarcodeResponse.IsValid = false;
                            validateBarcodeResponse.ErrorMessage = string.Format(
                                ValidateBarcodeErrorMessage.BarcodeContainerNotFound,
                                validateBarcodeRequest.ContainerId);
                            return validateBarcodeResponse;
                        }

                        // Check RemainingQty in UnsortCC
                        bool hasRemainingInRegister = await _unitOfWork.TransactionUnsortCCRepos
                            .AnyAsync(u =>
                                u.RegisterUnsortId == registerUnsort.RegisterUnsortId &&
                                u.IsActive == true &&
                                u.RemainingQty > 0);

                        if (!hasRemainingInRegister)
                        {
                            validateBarcodeResponse.IsValid = false;
                            validateBarcodeResponse.IsAllRemainingZero = true;
                            validateBarcodeResponse.ErrorMessage = ValidateBarcodeErrorMessage.HeaderCardIsAllRemainingZero;
                        ...
                        // Check RemainingQty in UnsortCC By Id
                        bool hasRemainingById = await _unitOfWork.TransactionUnsortCCRepos
                            .AnyAsync(u =>
                                u.RegisterUnsortId == registerUnsort.RegisterUnsortId &&
                                u.UnsortCCId == validateBarcodeRequest.UnSortCcId &&
                                u.IsActive == true &&
                                u.RemainingQty > 0);
                ...
                #endregion ValidateRemainingQty
