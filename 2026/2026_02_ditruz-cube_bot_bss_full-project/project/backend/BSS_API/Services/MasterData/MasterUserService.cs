...
namespace BSS_API.Services
{
    public class MasterUserService : IMasterUserService
    {
        ...
        {
            await _unitOfWork.UserRepos.ExecuteInTransactionAsync(async () =>
            {
                int newUserId = 0;

                var entity_row = await _unitOfWork.UserRepos.GetAsync(
                    item => item.DepartmentId == request.DepartmentId &&
                ...
                if (entity_row == null)
                {
                    var new_entity = new MasterUser
                    {
                        ...
                        IsInternal = request.UserName.Length > 30,
                ...
                var roleGroupId = request.RoleGroupId;

                var userRole_entity = await _unitOfWork.UserRoleRepos.GetAsync(
                    item => item.UserId == newUserId && item.RoleGroupId == roleGroupId,
                    tracked: true);
                ...
                else
                {
                    ...
                    _unitOfWork.UserRoleRepos.Update(userRole_entity);
                }
                await _unitOfWork.SaveChangeAsync();
            });
        }
        ...
        public async Task<IEnumerable<UserLoginDropdownData>?> GetUserLoginDropdownAsync()
        {
            var result = new List<UserLoginDropdownData>();

            var resultData = await _unitOfWork.UserRepos.GetUserLoginDropdownAsync();
            if (resultData.Any())
            {
                foreach (var itemRow in resultData)
                {
                    var newItem = new UserLoginDropdownData();
                    newItem.UserId = itemRow.UserId;
                    newItem.UserName = itemRow.UserName;
                    newItem.DepartmentId = itemRow.DepartmentId;
                    newItem.DepartmentName = itemRow.MasterDepartment.DepartmentName ?? string.Empty;
                    newItem.FirstName = itemRow.FirstName ?? string.Empty;
                    newItem.LastName = itemRow.LastName?? string.Empty;
                    newItem.RoleGroupName = itemRow.MasterUserRole.Where(x => x.IsActive == true)?.FirstOrDefault().MasterRoleGroup.RoleGroupName ?? string.Empty;
                    result.Add(newItem);
                }
            }
