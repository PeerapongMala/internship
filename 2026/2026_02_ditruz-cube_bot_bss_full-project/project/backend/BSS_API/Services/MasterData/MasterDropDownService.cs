...
namespace BSS_API.Services
{
    ...
    public class MasterDropDownService(IUnitOfWork unitOfWork) : IMasterDropDownService
    {
        ...
        public async Task<ICollection<DropDownItemResponse<MasterDenoUnsortCcViewData>>> GetDenominationByInstitutionIdAsync(SystemSearchRequest request)
        {
            ICollection<MasterDenoUnsortCcViewData> masterDenominationByInstitution =
                await unitOfWork.DenominationRepos.GetDenominationByInstitutionIdAsync(request);

            return masterDenominationByInstitution.ToDropDownItem(request.IncludeData);
        }

        public async Task<ICollection<DropDownItemResponse<MasterShift>>> GetDropDownShiftItemAsync(
            SystemSearchRequest request)
        {
            ICollection<MasterShift> masterShifts =
                await unitOfWork.ShiftRepos.GetMasterShiftWithSearchRequestAsync(request);
            return masterShifts.ToDropDownItem(request.IncludeData);
        }
    }
}