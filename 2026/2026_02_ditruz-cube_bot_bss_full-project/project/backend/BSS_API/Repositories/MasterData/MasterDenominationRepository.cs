...
namespace BSS_API.Repositories
{
    public class MasterDenominationRepository : GenericRepository<MasterDenomination>, IMasterDenominationRepository
    {
               ...
               .ToPagedDataAsync(
                   ...
                   applySearch: (query, keyword) =>
                   {
                       ...
                       return query.Where(x =>
                            (x.DenominationDesc ?? string.Empty).Contains(keyword)
                            || x.DenominationPrice.ToString().Contains(keyword)
                            || x.DenominationCode.ToString().Contains(keyword)
        ...
        private static readonly IReadOnlyDictionary<string, Expression<Func<MasterDenomination, object>>>
                MasterDenominationSortMap
                        ...
                        .OrdinalIgnoreCase)
                    {
                        ...
                        ["DenominationPrice"] = x => x.DenominationPrice,
                        ["DenominationDesc"] = x => x.DenominationDesc ?? string.Empty,
                        ["DenominationCurrency"] = x => x.DenominationCurrency,
                        ["IsActive"] = x => x.IsActive ?? false
                    };
        ...
        public async Task<List<MasterDenoUnsortCcViewData>> GetDenominationByInstitutionIdAsync(SystemSearchRequest request)
        {
            //int? receiveId = null;
            //var receiveCond = request.SearchCondition?
            //    .FirstOrDefault(c =>
            //        string.Equals(c.ColumnName, "receiveId", StringComparison.OrdinalIgnoreCase)
            //        && c.FilterValue != null);
            //if (receiveCond != null)
            //{
            //    receiveId = Convert.ToInt32(receiveCond.FilterValue);
            //}
            ...
            var containerCond = request.SearchCondition?
                .FirstOrDefault(c =>
                    string.Equals(c.ColumnName, "ContainerId", StringComparison.OrdinalIgnoreCase)
                    && c.FilterValue != null);
            if (containerCond != null)
            {
                containerId = containerCond.FilterValue?.ToString();
            }

            var query =
                from d in _db.MasterDenominations.AsNoTracking()
                where _db.ReceiveCbmsDataTransactions.AsNoTracking()
            .Any(r => r.DenominationId == d.DenominationId
              && (!instId.HasValue || r.InstitutionId == instId.Value)
              && (containerId == null || r.ContainerId == containerId)
              && r.RemainingQty > 0)
                select new MasterDenoUnsortCcViewData
                {
                    DenoId = d.DenominationId,
