...
namespace BSS_API.Repositories
{
            ...
            ITransactionPreparationRepository
    {
        ...
        {
            // Validate input
            if (string.IsNullOrWhiteSpace(headerCard))
            {
                return null;
            }
            ...
            // เงื่อนไขที่ 1: ห้ามซ้ำภายใน 2 วัน
            var result = await _dbSet
                ...
                .Where(w => w.HeaderCardCode == headerCard &&
                            ...
                            w.IsActive == true &&
                            (!departmentId.HasValue || (w.TransactionContainerPrepare != null &&
                                                         w.TransactionContainerPrepare.DepartmentId == departmentId.Value)))
                .AsNoTracking()
                .FirstOrDefaultAsync();
            ...
            // เงื่อนไขที่ 2: ห้ามซ้ำในระบบ (status != 7)
            return await _dbSet
                .Include(x => x.TransactionContainerPrepare)
                .Where(w => w.HeaderCardCode == headerCard &&
                            w.StatusId != 7 &&
        ...
        #region Report
        public async Task<IEnumerable<DropdownItemResponse>> GetHeaderCardByCriteriaAsync(string machineId, DateTime date)
        {
            var query = _db.TransactionPreparation
                .AsNoTracking()
                .Where(x => x.IsActive == true && x.PrepareDate.Date == date.Date);

            if (!string.IsNullOrEmpty(machineId) && machineId != "all")
            {
                if (int.TryParse(machineId, out int mId))
                {
                    query = query.Where(x => x.TransactionContainerPrepare.MachineId == mId);
                }
            }
            ...
            return await query
                ...
                {
                    // ใช้ HeaderCardCode เป็นทั้ง Text และ Value
                    // หรือถ้ามี ID ของมันโดยเฉพาะก็สามารถใส่ใน Id ได้
                    Text = x.HeaderCardCode,
                    Value = x.HeaderCardCode,
                    AdditionalData = new
                    {
                        InstitutionName = x.MasterInstitution.InstitutionNameTh,
                        InstId = x.InstId
                    }
                })
                .Distinct()
        ...
        public async Task<ICollection<TransactionPreparation>> GetPrepareSingleHeaderCardAsync(reportSingleHeaderCardRequest request)
        {
            // 1. Parse 'headercardSelect' จาก string เป็น List<string>
            var selectedIds = request.headercardSelect?
            ...
            // 2. สร้าง Query พร้อมดึงข้อมูลจาก Navigation Properties
            var query = _db.TransactionPreparation
                .Include(p => p.TransactionContainerPrepare)
                    .ThenInclude(c => c.MasterMachine)
                .Include(p => p.TransactionContainerPrepare)
                    .ThenInclude(c => c.MasterBanknoteType)
                .Include(p => p.MasterDenomination) // เพื่อเอา DenoName และ Series
                .Include(p => p.MasterInstitution)  // ชื่อธนาคาร
                .Include(p => p.MasterCashCenter)   // ศูนย์เงินสด
                .Include(p => p.CreatedByUser)      // ผู้นับคัด (Operator-Prepare/Sorter)
                .Include(p => p.TransactionReconcileTran) // ข้อมูลผลการนับ (ดี/เสีย/Reject)
                .Where(x => selectedIds.Contains(x.HeaderCardCode) &&
                            x.TransactionContainerPrepare.DepartmentId == request.DepartmentId &&
                            x.IsActive == true)
                .AsNoTracking();

            // 3. กรองตาม MachineId (ถ้าไม่ใช่ "all")
            if (!string.IsNullOrEmpty(request.machineId) && request.machineId.ToLower() != "all")
            {
                if (int.TryParse(request.machineId, out int mid))
                {
                    query = query.Where(x => x.TransactionContainerPrepare.MachineId == mid);
                }
            }

            // 4. กรองตามวันที่ (เปรียบเทียบเฉพาะส่วนวันที่)
            // สร้าง CultureInfo ของไทย (พ.ศ.)
            var thaiCulture = new System.Globalization.CultureInfo("th-TH");

            if (DateTime.TryParse(request.date, thaiCulture, System.Globalization.DateTimeStyles.None, out DateTime filterDate))
            {
                // เมื่อ Parse ด้วย thaiCulture: "01/07/2569" จะกลายเป็น DateTime ปี ค.ศ. 2026 โดยอัตโนมัติ
                query = query.Where(x => x.PrepareDate.Date == filterDate.Date);
            }

            return await query.ToListAsync();
        }

        public async Task<ICollection<TransactionPreparation>> GetPrepareMultiHeaderCardAsync(reportMultiHeaderCardRequest request)
        {
            // 1. Parse 'headercardSelect' จาก string เป็น List<string>
            //var selectedIds = request.headercardSelect?
            //    .Split(',', StringSplitOptions.RemoveEmptyEntries)
            ...
            var query = _db.TransactionPreparation
                ...
                .Include(p => p.CreatedByUser)      // ผู้นับคัด (Operator-Prepare/Sorter)
                .Include(p => p.TransactionReconcileTran) // ข้อมูลผลการนับ (ดี/เสีย/Reject)
                .Where(x => x.TransactionContainerPrepare.DepartmentId == request.DepartmentId &&
                            x.IsActive == true)
                .AsNoTracking();
        ...
        public async Task<List<PrepareHeaderCardResponse>> GetUnmatchedPrepareAsync(
            int departmentId, int? machineId, string? bnTypeCode = null)
        {
            var query = _db.TransactionPreparation
                ...
                .Include(x => x.TransactionContainerPrepare)
                .Include(x => x.MasterDenomination)
                .Where(x => x.TransactionContainerPrepare.DepartmentId == departmentId
                         && x.TransactionContainerPrepare.IsActive == true
                         && x.StatusId == 9
                         && x.IsActive == true
                         && x.IsReconcile != true
                         && x.IsMatchMachine != true);

            if (machineId.HasValue)
            {
                query = query.Where(x => x.TransactionContainerPrepare.MachineId == machineId.Value);
            }

            if (!string.IsNullOrEmpty(bnTypeCode))
            {
                query = query.Where(x => x.TransactionContainerPrepare.MasterBanknoteType.BssBanknoteTypeCode == bnTypeCode);
            }

            return await query
                .OrderByDescending(x => x.PrepareDate)
                .Select(x => new PrepareHeaderCardResponse
                {
                    PrepareId = x.PrepareId,
                    HeaderCardCode = x.HeaderCardCode,
                    PrepareDate = x.PrepareDate,
                    DenominationPrice = x.MasterDenomination != null
                        ? x.MasterDenomination.DenominationPrice
                        : (int?)null,
                    DenoId = x.DenoId
                })
                .ToListAsync();
