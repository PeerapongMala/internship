        ...
        : GenericRepository<TransactionSendUnsortCC>(db), ITransactionSendUnsortCCRepository
    {
                    ...
                    ContainerData = x.TransactionSendUnsortData
                        ...
                        .Select(cc => new ContainerBySendUnsortIdResponse
                        {
                            ...
                            BanknoteQty = cc.TransactionRegisterUnsort.TransactionUnsortCCs
                                ...
                                .Sum(m => (int?)m.BanknoteQty - (int?)(m.AdjustQty ?? 0)) ?? 0,
            ...
            CancellationToken ct = default)
        {
            ...
            if (rowData != null)
            {
                ...
                {
                    registerData.StatusId = 8; // กลับเป็นสถานะว่าง/ปกติ
        ...
        public async Task<bool> UpdateSendUnsortStatusAsync(int SendUnsortId, int UserId, int statusId, string? note, CancellationToken ct = default)
        {
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    ...
                    // เช็คสิทธิ์ (ห้ามคนสร้างเป็นคนจัดการสถานะรับ/คืนเอง)
                    if ((statusId == 6 || statusId == 5 || statusId == 31) && rowData.CreatedBy == UserId)
                        return false;

                    rowData.StatusId = statusId;
                    rowData.UpdatedBy = UserId;
                    rowData.UpdatedDate = now;
                    if (statusId == 6) rowData.ReceivedDate = now;
                    if (statusId == 5 || statusId == 31) rowData.Remark = note;

                    // 2. ดึงข้อมูลตารางย่อยทั้งหมด
                    var listData = await _db.TransactionSendUnsortDatas
                        .Where(x => x.SendUnsortId == SendUnsortId)
                        .ToListAsync(ct);

                    var registerUnsortIds = listData.Select(x => x.RegisterUnsortId).ToList();

                    // ดึงข้อมูล Register และ UnsortCC ออกมาล่วงหน้าเพื่อลด Query ใน Loop
                    var allRegisterData = await _db.TransactionRegisterUnsorts
                        .Where(x => registerUnsortIds.Contains(x.RegisterUnsortId) && x.IsActive == true)
                        .ToListAsync(ct);

                    var allUnsortCCs = await _db.TransactionUnsortCCs
                        .Where(x => registerUnsortIds.Contains(x.RegisterUnsortId) && x.IsActive == true)
                        .ToListAsync(ct);

                    // 3. เริ่ม Loop จัดการข้อมูล
                    foreach (var registerData in allRegisterData)
                    {
                        // --- กรณีส่งคืน (Status 31) ---
                        if (statusId == 31)
                        {
                            // เงื่อนไข: ต้องเป็น Status 6 ถึงจะทำรายการต่อได้
                            if (registerData.StatusId == 6)
                            {
                                var relatedCCs = allUnsortCCs.Where(x => x.RegisterUnsortId == registerData.RegisterUnsortId).ToList();

                                // ตรวจสอบว่าจำนวนเงิน/จำนวนใบยังอยู่ครบ (ไม่มีการดึงไปใช้งาน)
                                foreach (var cc in relatedCCs)
                                {
                                    if (cc.BanknoteQty != (cc.RemainingQty + cc.AdjustQty))
                                    {
                                        return false; // ถ้ามีการใช้งานไปแล้ว ไม่อนุญาตให้ส่งคืนทั้งใบนำส่ง
                                    }

                                    // อัปเดตค่ากลับ (ถ้าต้องการความชัวร์ว่าค่าจะเท่ากัน)
                                    // cc.BanknoteQty = cc.RemainingQty; 
                                }

                                registerData.StatusId = 31;
                                registerData.UpdatedBy = UserId;
                                registerData.UpdatedDate = now;
                            }
                            else
                            ...
                            continue; // ข้ามไปรายการถัดไปเลยสำหรับสถานะ 31
                        }

                        // --- กรณีสถานะอื่นๆ (5, 6, ฯลฯ) ---
                        registerData.StatusId = statusId;
                        registerData.UpdatedBy = UserId;
                        registerData.UpdatedDate = now;

                        if (statusId == 6) registerData.ReceivedDate = now;

                        // เช็คสิทธิ์ระดับรายการย่อย
                        if ((statusId == 6 || statusId == 5) && registerData.CreatedBy == UserId)
                            return false;
                    }
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    ...
                    if (request.RemainingQty > rowData.RemainingQty)
                    {
                        ...
                        throw new Exception("Warning:ไม่สามารถอัปเดตจำนวนมัดได้ จำนวนมัดที่แก้ไขต้องไม่มากกว่าจำนวนมัดเดิม");
                    }
                    // ------------------------------------------------------
                    ...
                    if (allItemsInGroup.All(x => x.RemainingQty == 0))
                    {
                        ...
                        // คำนวณยอดที่ Prepare ไปแล้วตามสูตร: sum(banknote) - (sum(remaining) + sum(adjust))
                        // ซึ่งถ้า sum(remaining) เป็น 0 แล้ว จะเหลือแค่: sum(banknote) - sum(adjust)
                        var totalBanknote = allItemsInGroup.Sum(x => x.BanknoteQty);
                        var totalAdjust = allItemsInGroup.Sum(x => x.AdjustQty ?? 0);
                        var preparedQty = totalBanknote - (0 + totalAdjust);

                        bool shouldCloseContainer = false;

                        if (preparedQty == 0)
                        {
                            // เงื่อนไข: ไม่มีมัดที่ prepare เลย
                            shouldCloseContainer = true;
                            alertMessage = "Warning:ภาชนะนี้สิ้นสุดแล้ว ผู้ใช้งานสามารถนำภาชนะนี้ไปลงทะเบียนใหม่ได้";
                        }
                        else
                        {
                            // กรณีที่ 2: มีการ prepare ไปบ้างแล้ว -> เช็คว่า "ทุกมัดที่เตรียม" ต้องเป็น Status 19 ทั้งหมด
                            // วิธีเช็คที่แม่นยำคือ: "ต้องไม่มีมัดไหน (IsActive) ที่สถานะยังไม่ใช่ 19"
                            var hasUnconfirmedItems = await _db.TransactionPreparation
                                .AnyAsync(p => p.TransactionUnsortCC.RegisterUnsortId == request.RegisterUnsortId
                                            && p.IsActive == true
                                            && p.StatusId != 19, ct);

                            if (!hasUnconfirmedItems)
                            {
                                // ถ้าไม่มีตัวที่ยังไม่ยืนยัน (แปลว่าทุกตัวเป็น 19 หมดแล้ว)
                                shouldCloseContainer = true;
                                alertMessage = "Warning:ภาชนะนี้สิ้นสุดแล้ว ผู้ใช้งานสามารถนำภาชนะนี้ไปลงทะเบียนใหม่ได้";
                            }
                            else
                            {
                                // ถ้ายังมีบางมัดที่ Status != 19
                                alertMessage = "Warning:ผู้ใช้งานสามารถนำภาชนะนี้ไปลงทะเบียนใหม่ได้เมื่อได้รับการยืนยันผลนับคัดจาก BMS แล้วทั้งหมด";
                            }
                        }

                        if (shouldCloseContainer)
                        {
                            // อัปเดตตาราง RegisterUnsort เป็นสถานะ 7 (สิ้นสุด)
                            ...
                            {
                                registerUnsort.StatusId = 7; // 7 = สิ้นสุด
                                registerUnsort.UpdatedBy = request.CreatedBy;
                                registerUnsort.UpdatedDate = now;
                            }

                            // อัปเดตตารางเชื่อมโยง (bss_txn_send_unsort_data) ให้ IsActive = false
                            ...
                            // เช็คเพื่อปิด SendUnsortCC (ตารางแม่) ถ้าทุกตู้ในรอบนั้นปิดหมดแล้ว
                            var sendUnsortId = sendUnsortRecords.FirstOrDefault()?.SendUnsortId;
                            if (sendUnsortId.HasValue)
                            {
                                var hasOtherActive = await _db.TransactionSendUnsortDatas
                                    .AnyAsync(x => x.SendUnsortId == sendUnsortId && x.IsActive == true
                                                && x.RegisterUnsortId != request.RegisterUnsortId, ct);

                                if (!hasOtherActive)
                                {
                                    var masterSend = await _db.TransactionSendUnsortCCs
                                        .FirstOrDefaultAsync(x => x.SendUnsortId == sendUnsortId, ct);
                                    if (masterSend != null)
                                    {
                                        masterSend.StatusId = 7; // ปิดใบนำส่งด้วย
                                        masterSend.UpdatedBy = request.CreatedBy;
                                        masterSend.UpdatedDate = now;
                                    }
                                }
                            }
                        }
