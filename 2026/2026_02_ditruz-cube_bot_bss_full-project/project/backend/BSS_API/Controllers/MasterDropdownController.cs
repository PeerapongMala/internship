        ...
        : BaseController(share)
    {
        ...
        public async Task<IActionResult> GetMasterDropDown(SystemSearchRequest request)
        {
            ...
            switch (request.TableName)
            {
                ...
                case EntityNameConstants.MasterDenominationWithBank:
                    return ApiSuccess(await masterDropDownService.GetDenominationByInstitutionIdAsync(request));
                case EntityNameConstants.MasterShift:
                    return ApiSuccess(await masterDropDownService.GetDropDownShiftItemAsync(request));
                default:
