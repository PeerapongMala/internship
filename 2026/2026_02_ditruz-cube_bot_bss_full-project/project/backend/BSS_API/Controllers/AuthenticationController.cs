...
namespace BSS_API.Controllers
{
    ...
    public class AuthenticationController : BaseController
    {
        ...
        [HttpGet("CheckUserSessionLoginActive")]
        public async Task<IActionResult> CheckUserSessionLoginActive([Required] string usernameId)
        {
            var userResult = await _authorizationService.CheckUserSessionLoginActiveAsync(usernameId);
            if (userResult == null)
            {
                return ApiDataNotFound("ไม่พบข้อมูลผู้ใช้งานในระบบ");
            }
        ...
        [HttpGet("GetUserLoginDropdown")]
        public async Task<IActionResult> GetUserLoginDropdown()
        {
            var userResult = await _masterUserService.GetUserLoginDropdownAsync();
            if (userResult == null)
            {
                return ApiDataNotFound("ไม่พบข้อมูลผู้ใช้งานในระบบ");
            }
        ...
        [HttpPost("CheckUserSessionByMachine")]
        public async Task<IActionResult> CheckUserSessionByMachine(CheckUserSessionByMachineActiveRequest request)
        {
            var userResult = await _authorizationService.CheckUserSessionByMachineAsync(request);
            if (userResult == null)
            {
                return ApiDataNotFound("ไม่พบข้อมูลผู้ใช้งานในระบบ");
            }
